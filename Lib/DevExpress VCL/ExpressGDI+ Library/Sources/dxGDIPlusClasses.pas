...
{           GDI+ Library                                             }
{                                                                    }
{           Copyright (c) 2002-2018 Developer Express Inc.           }
{           ALL RIGHTS RESERVED                                      }
{                                                                    }
...
type
  ...
  TdxGPPenLineCapStyle = (gpcsFlat, gpcsSquare, gpcsRound);
  ...
  public
    ...
    procedure AddString(AText: PWideChar; AFont: TdxGPFont; AFormat: TdxGPStringFormat; const ARect: TdxRectF); overload;
    procedure AddString(AText: PWideChar; AFont: TFont; AEmSize: Single; const ARect: TRect); overload;
    //
...
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
  ...
  ATrimming: TdxGpStringTrimming = StringTrimmingNone); overload;
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
...
const
  ...
  PenLineStyleToGpLineCap: array[TdxGPPenLineCapStyle] of TdxGpLineCap = (LineCapFlat, LineCapSquare, LineCapRound);
...
begin
  ...
  begin
    ...
    GdipCheck(GdipSetPenDashStyle(APenHandle, PenStyleToDashStyle[APenStyle]));
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (SrcW > 1) and (DstW > SrcW) then
        SrcW := SrcW - 1;
      if (SrcH > 1) and (DstH > SrcH) then
        SrcH := SrcH - 1;
    end;
  end;
...
procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string;
  ...
  ARendering: TdxGpTextRenderingHint; ATrimming: TdxGpStringTrimming);
begin
  dxGPDrawText(AGraphics, AText, cxRectF(ARect), AFont, ATextColor, AHorzAlignment, AVertAlignment, AWordWrap, ARendering, ATrimming);
end;

procedure dxGPDrawText(AGraphics: TdxGPCanvas; const AText: string; const ARect: TdxRectF; AFont: TFont;
  ATextColor: TdxAlphaColor; AHorzAlignment: TAlignment; AVertAlignment: TVerticalAlignment; AWordWrap: Boolean;
  ARendering: TdxGpTextRenderingHint; ATrimming: TdxGpStringTrimming);
var
  AGpBrush: GpBrush;
  AGpFont: GpFont;
...
begin
  ...
  AStringFormat := TdxGPStringFormat.Create(Integer(dxGpWordWrapFlagsMap[AWordWrap]));
  try
    ...
    try
      ...
      GdipCheck(GdipDrawString(AGraphics.Handle, PWideChar(AText), -1, AGpFont, @AGpRectF, AStringFormat.Handle, AGpBrush));
    finally
...
procedure TdxGPPath.AddString(AText: PWideChar; AFont: TdxGPFont; AFormat: TdxGPStringFormat; const ARect: TdxRectF);
var
  AFormatHandle: GpStringFormat;
  AGpRectF: TdxGpRectF;
begin
  AGpRectF.X := ARect.Left;
  AGpRectF.Y := ARect.Top;
  AGpRectF.Height := ARect.Bottom - ARect.Top;
  AGpRectF.Width := ARect.Right - ARect.Left;

  if AFormat <> nil then
    AFormatHandle := AFormat.Handle
  else
    AFormatHandle := nil;

  GdipCheck(GdipAddPathString(Handle, AText, -1, AFont.NativeFont, AFont.Style, AFont.Size, @AGpRectF, AFormatHandle));
end;
...
begin
  ...
  GdipCheck(GdipCreateFontFamilyFromName(PWideChar((AFont.Name)), nil, AFontFamily));
  try
    AGpRectF := dxGpMakeRectF(ARect);
    GdipCheck(GdipAddPathStringI(Handle, AText, -1, AFontFamily,
      dxFontStylesToGpFontStyles(AFont.Style), AEmSize, @AGpRectF, AStringFormat));
  finally
...
begin
  if dxGpCreatePen(APen, APenColor, APenWidth, APenStyle, APenColorAlpha) then
  begin
    GdipCheck(GdipDrawCurveI(Handle, APen, @APoints[0], Length(APoints)));
    GdipCheck(GdipDeletePen(APen));
  end;
end;

procedure TdxGPCanvas.Curve(const APoints: array of TPoint; APenColor: TdxAlphaColor;
  APenWidth: Single = 1; APenStyle: TPenStyle = psSolid);
var
  AColor: TColor;
  AAlpha: Byte;
begin
  AColor := dxAlphaColorToColor(APenColor, AAlpha);
  Curve(APoints, AColor, APenWidth, APenStyle, AAlpha);
end;
...
begin
  if dxGpCreatePen(APen, APenColor, APenWidth, APenStyle, APenColorAlpha) then
  begin
    GdipCheck(GdipDrawCurve(Handle, APen, @APoints[0], Length(APoints)));
    GdipCheck(GdipDeletePen(APen));
  end;
end;
...
var
  APenRGBColor, ABrushRGBColor: TColor;
  APenAlpha, ABrushAlpha: Byte;
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Ellipse(R, APenRGBColor, ABrushRGBColor, APenWidth, APenStyle, APenAlpha, ABrushAlpha);
end;

procedure TdxGPCanvas.Ellipse(R: TdxRectF; APenColor, ABrushColor: TdxAlphaColor;
  APenWidth: Single = 1; APenStyle: TPenStyle = psSolid);
begin
  Ellipse(R, dxAlphaColorToColor(APenColor), dxAlphaColorToColor(ABrushColor),
    APenWidth, APenStyle, dxGetAlpha(APenColor), dxGetAlpha(ABrushColor));
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  Line(X1, Y1, X2, Y2, APenRGBColor, APenWidth, APenStyle, APenAlpha);
end;

procedure TdxGPCanvas.Line(X1, Y1, X2, Y2: Single; APenColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  APen: GpPen;
begin
  if dxGpCreatePen(APen, dxAlphaColorToColor(APenColor), APenWidth, APenStyle, dxGetAlpha(APenColor)) then
  begin
    GdipCheck(GdipDrawLine(Handle, APen, X1, Y1, X2, Y2));
    GdipCheck(GdipDeletePen(APen));
  end;
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Path(APath, APenRGBColor, ABrushRGBColor, APenWidth, APenStyle, APenAlpha, ABrushAlpha);
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Pie(R, AStartAngle, ASweepAngle, APenRGBColor, ABrushRGBColor, APenWidth, APenStyle, APenAlpha, ABrushAlpha);
end;

procedure TdxGPCanvas.Pie(R: TdxRectF; AStartAngle, ASweepAngle: Single; APenColor, ABrushColor: TdxAlphaColor;
  APenWidth: Single; APenStyle: TPenStyle);
var
  ABrush: GpBrush;
  APen: GpPen;
  APenRGBColor, ABrushRGBColor: TColor;
...
begin
  ...
  if dxGpCreateBrush(ABrush, ABrushRGBColor, ABrushAlpha) then
  begin
    GdipCheck(GdipFillPie(Handle, ABrush, R.Left, R.Top, cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
    GdipCheck(GdipDeleteBrush(ABrush));
  end;

  if dxGpCreatePen(APen, APenRGBColor, APenWidth, APenStyle, APenAlpha) then
  begin
    GdipCheck(GdipDrawPie(Handle, APen, R.Left, R.Top, cxRectWidth(R), cxRectHeight(R), -AStartAngle, -ASweepAngle));
    GdipCheck(GdipDeletePen(APen));
  end;
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Polygon(APoints, APenRGBColor, ABrushRGBColor, APenWidth, APenStyle, APenAlpha, ABrushAlpha);
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Polygon(APoints, APenRGBColor, ABrushRGBColor, APenWidth, TPenStyle(APenStyle), APenAlpha, ABrushAlpha);
end;
...
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  Polyline(APoints, APenRGBColor, APenWidth, APenStyle, APenAlpha);
end;
...
var
  ABrush: GpBrush;
begin
  if TdxAlphaColors.IsTransparentOrEmpty(ABrushColor) then
    Exit;

  GdipCheck(GdipCreateSolidFill(ABrushColor, ABrush));
  GdipCheck(GdipFillRectangle(Handle, ABrush, R.Left, R.Top, cxRectWidth(R), cxRectHeight(R)));
  GdipCheck(GdipDeleteBrush(ABrush));
end;
...
var
  ...
  APen: GpPen;
begin
  if ABrushColor <> TdxAlphaColors.Transparent then
  begin
    GdipCheck(GdipCreateSolidFill(ABrushColor, ABrush));
    GdipCheck(GdipFillRectangle(Handle, ABrush, R.Left, R.Top, cxRectWidth(R), cxRectHeight(R)));
    GdipCheck(GdipDeleteBrush(ABrush));
  ...
  if (APenColor <> TdxAlphaColors.Transparent) and (APenWidth > 0) then
  begin
    GdipCheck(GdipCreatePen1(APenColor, APenWidth, guPixel, APen));
    GdipCheck(GdipSetPenDashStyle(APen, PenStyleToDashStyle[APenStyle]));
    GdipCheck(GdipDrawRectangle(Handle, APen, R.Left, R.Top, cxRectWidth(R), cxRectHeight(R)));
    GdipCheck(GdipDeletePen(APen));
  end;
end;
...
procedure TdxGPCanvas.Rectangle(R: TRect; APenColor, ABrushColor: TdxAlphaColor;
  APenWidth: Single = 1; APenStyle: TPenStyle = psSolid);
var
  APenRGBColor, ABrushRGBColor: TColor;
  APenAlpha, ABrushAlpha: Byte;
begin
  APenRGBColor := dxAlphaColorToColor(APenColor, APenAlpha);
  ABrushRGBColor := dxAlphaColorToColor(ABrushColor, ABrushAlpha);
  Rectangle(R, APenRGBColor, ABrushRGBColor, APenWidth, APenStyle, APenAlpha, ABrushAlpha);
end;
...
procedure TdxGPCanvas.Region(ARegion: TdxGPRegion; APenColor, ABrushColor: TdxAlphaColor; APenWidth: Single = 1;
  APenStyle: TPenStyle = psSolid);
var
  ABrush: GpBrush;
begin
  GdipCheck(GdipCreateSolidFill(ABrushColor, ABrush));
  GdipCheck(GdipFillRegion(Handle, ABrush, ARegion.Handle));
  GdipCheck(GdipDeleteBrush(ABrush));
end;
