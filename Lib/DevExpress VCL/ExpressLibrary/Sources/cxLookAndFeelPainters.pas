...
type
  ...
  ['{76692C28-2C4B-48B9-EAC6-8C3A7D1BC05E}']
  ...
  public
    ...
    function GridLikeControlDefaultUseOddEventStyle: Boolean; virtual;
    // Layout View
    ...
    procedure DrawSizeGrip(ACanvas: TcxCanvas; const ARect: TRect; ABackgroundColor: TColor = clDefault; ACorner: TdxCorner = coBottomRight); virtual;
    ...
    procedure DrawPanelContent(ACanvas: TcxCanvas; const ABorderRect: TRect; ABorder: Boolean); virtual;
    ...
    procedure DrawScaledSplitter(ACanvas: TcxCanvas; const ARect: TRect;
      AHighlighted, AClicked, AHorizontal: Boolean; AScaleFactor: TdxScaleFactor); virtual;
  ...
  public
    ...
    // scrollbars
    procedure DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean;
  ...
  public
    ...
    function TabBorderSize(AVertical: Boolean): Integer; override;
    // scrollbars
    procedure DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean; R: TRect;
  ...
  public
    ...
    class procedure DrawScaledThemeBackground(ATheme: TdxTheme; DC: HDC;
      APartId, AStateId: Integer; const R: TRect; AScaleFactor: TdxScaleFactor; AForceScale: Boolean = False);
...
procedure TcxOffice11LookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean; R: TRect;
  APart: TcxScrollBarPart; AState: TcxButtonState; AScaleFactor: TdxScaleFactor);
var
  APainter: TcxCustomLookAndFeelPainter;
begin
  if cxLookAndFeelPaintersManager.GetPainter(lfsNative, APainter) then
    APainter.DrawScaledScrollBarPart(ACanvas, AHorizontal, R, APart, AState, AScaleFactor)
  else
...
procedure TcxFlatLookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean; R: TRect;
  APart: TcxScrollBarPart; AState: TcxButtonState; AScaleFactor: TdxScaleFactor);
begin
...
procedure TcxUltraFlatLookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean;
...
procedure TcxStandardLookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCanvas;
...
begin
  if AKind <> ikNone then
    cxDrawImage(ACanvas, R, nil, cxIndicatorImages, Ord(AKind) - 1, ifmNormal, idmNormal, False, nil, AScaleFactor);
end;
...
begin
  ...
  SearchButtonGlyph.StretchDraw(ACanvas.Handle,
    cxRectCenter(R, AScaleFactor.Apply(cxRectSize(AGlyphRect))),
...
begin
  if ABounds.Left = AThumbBounds.Left then
    DrawScaledToggleSwitchState(ACanvas, ABounds, AState, False, AScaleFactor)
  else
    if ABounds.Right = AThumbBounds.Right then
      DrawScaledToggleSwitchState(ACanvas, ABounds, AState, True, AScaleFactor)
    else
    begin
      ACenterThumb := AThumbBounds.Left + cxRectWidth(AThumbBounds) div 2;
      AExcludeRect := ABounds;
      AExcludeRect.Left := ACenterThumb;
      InternalDrawPart(AExcludeRect, True);
      AExcludeRect := ABounds;
      AExcludeRect.Right := ACenterThumb;
      InternalDrawPart(AExcludeRect, False);
    end;
end;
...
function TcxCustomLookAndFeelPainter.GridLikeControlDefaultUseOddEventStyle: Boolean;
begin
...
procedure TcxCustomLookAndFeelPainter.DrawNavigatorScaledButtonGlyph(ACanvas: TcxCanvas; AImageList: TCustomImageList;
...
procedure TcxCustomLookAndFeelPainter.DrawScaledSplitter(ACanvas: TcxCanvas;
  const ARect: TRect; AHighlighted, AClicked, AHorizontal: Boolean; AScaleFactor: TdxScaleFactor);
...
begin
  ...
  if (ATheme <> 0) and IsWinVistaOrLater then
  begin
    ARect := cxRectSetOrigin(cxRect(GetScaledCommandLinkGlyphSize(AScaleFactor)), AGlyphPos);
...
procedure TcxWinXPLookAndFeelPainter.DrawScaledSplitter(ACanvas: TcxCanvas;
  const ARect: TRect; AHighlighted, AClicked, AHorizontal: Boolean; AScaleFactor: TdxScaleFactor);
begin
  // do nothing
end;
...
procedure TcxWinXPLookAndFeelPainter.DrawScaledScrollBarPart(ACanvas: TcxCanvas; AHorizontal: Boolean; R: TRect;
  APart: TcxScrollBarPart; AState: TcxButtonState; AScaleFactor: TdxScaleFactor);
const
...
class procedure TcxWinXPLookAndFeelPainter.DrawScaledThemeBackground(ATheme: TdxTheme;
  DC: HDC; APartId, AStateId: Integer; const R: TRect; AScaleFactor: TdxScaleFactor; AForceScale: Boolean = False);
var
  ABitmap: TcxBitmap32;
  APartSize: TSize;
begin
...
begin
  DrawArrow(ACanvas, cxRectInflate(R, -1, -1), ArrowDirections[AState = dxbcbsPressed], BreadcrumbEditNodeTextColor(AState));
end;
