...
type
  ...
  protected
    ...
    property IsVectorTexture: Boolean read FTextureIsVector;
  ...
  published
    ...
    property TextColor: TColor read GetTextColor write SetTextColor default clDefault;
...
begin
  ...
  begin
    if Supports(ATexture, IdxVectorImage) or (ATexture.ImageDataFormat = dxImageGif) then
...
begin
  FTextureIsVector := Supports(Texture, IdxVectorImage);
  Changed;
end;
...
function TdxSkinElement.GetTextColor: TColor;
begin
...
begin
  ...
  begin
    if AValue then
      FCache := TdxSkinElementCacheList.Create
    else
      FreeAndNil(FCache);
  end;
end;
...
begin
  ...
  begin
    if (AColor1 = AColor2) or not cxColorIsValid(AColor2) then
      dxGpFillRect(ACanvas.Handle, R, dxColorToAlphaColor(AColor1, AAlpha))
    else
...
begin
  if FCacheOpaque <> nil then
    ...
    begin
      ...
      try
        GdipCheck(GdipDrawImageRectI(AGraphics.Handle, FCache, R.Left, R.Top, R.Right - R.Left, R.Bottom - R.Top));
      finally
...
var
  ACacheItem: TdxSkinElementCache;
begin
  if not TryGet(AElement, R, AState, AImageIndex, ACacheItem) then
  begin
    if FData.Count = Capacity then
    begin
      FData.Move(0, FData.Count - 1);
      ACacheItem := TdxSkinElementCache(FData.Last);
    end
    else
    begin
      ACacheItem := TdxSkinElementCache.Create;
      FData.Add(ACacheItem);
    end;
  end;
  ACacheItem.DrawEx(DC, AElement, R, AScaleFactor, AState, AImageIndex);
end;
