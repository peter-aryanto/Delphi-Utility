...
uses
  Types, SysUtils, TypInfo, Windows, Messages, Graphics, Controls, Forms, Classes, cxGraphics;
...
function GetOffice11FingerElementSize: Integer;
...
uses
  dxThemeManager, dxUxTheme, dxThemeConsts, dxDPIAwareUtils, cxGeometry;
...
begin
  ASize := GetOffice11FingerElementSize;
  AOffset := dxSystemScaleFactor.Apply(1);
  with ARect do
...
begin
  ...
  ARect := Rect(ARect.Right - 12, ARect.Bottom - dxSystemScaleFactor.Apply(3), ARect.Right, ARect.Bottom);
  Office11DrawFingerElements(DC, ARect, False, ABrush1, ABrush2); // 3
  Inc(ARect.Left, 4);
  OffsetRect(ARect, 0, -GetOffice11FingerElementSize);
...
function GetOffice11FingerElementSize: Integer;
begin
  Result := dxSystemScaleFactor.Apply(4);
end;
