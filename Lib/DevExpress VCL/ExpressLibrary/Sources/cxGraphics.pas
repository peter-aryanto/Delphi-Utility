...
{$ENDIF}
  ...
  protected
    ...
    procedure CheckFontSize(ALastSuitableSize, AOriginalSize, AMinSize: Integer); virtual;
    function GetMinimalFontSize: Integer; virtual; abstract;
    function GetTextWidth(const AText: string): Integer; virtual; abstract;
  public
    constructor Create;
    destructor Destroy; override;
    procedure Calculate(AAvailableWidth: Integer; const AText: string); virtual;
  ...
  protected
    ...
    function GetMinimalFontSize: Integer; override;
  ...
  protected type
   TImageListCacheHelper = class(TComponent)
    strict private type
      ...
      TImageID = packed record
        Index: Integer;
        DrawMode: TcxImageDrawMode;
        UseLeftBottomPixelAsTransparent: Boolean;
        SmoothImage: Boolean;
        PaletteID: TGUID;
        constructor Create(AImageIndex: Integer; ADrawMode: TcxImageDrawMode;
          AUseLeftBottomPixelAsTransparent: Boolean; const APalette: IdxColorPalette; ASmoothImage: Boolean);
        function IsEqual(const AID: TImageID): Boolean;
      end;

      TImages = class(TObjectDictionary<TImageID, TdxGPImage>)
      protected type
        TImageComparer = class(TEqualityComparer<TImageID>)
        public
          function Equals(const Left, Right: TImageID): Boolean; override;
          function GetHashCode(const Value: TImageID): Integer; override;
        end;
      public
    ...
    protected
      ...
      procedure PrepareImage(AImage: TdxGPImage; const ASize: TSize; const AImageID: TImageID; APalette: IdxColorPalette);
    public
...
{$ENDREGION}
  ...
  public
    class procedure Draw(ACanvas: TcxCanvas; const ABounds: TRect; AImages: TCustomImageList; AIndex: Integer;
      ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean; APalette: IdxColorPalette; ASmoothImage: Boolean); static;
...
{$IFDEF DELPHI22}
  System.Hash,
{$ENDIF}
  Forms, Messages, Math, Contnrs, StrUtils,
  cxControls, dxUxTheme, dxOffice11, dxThemeConsts, dxThemeManager, cxDrawTextUtils, dxSVGImage, dxDPIAwareUtils, dxTypeHelpers,
  dxHash, Consts;
...
procedure cxDrawImage(ACanvas: TcxCanvas; const ARect: TRect; AGlyph: TGraphic;
...
procedure cxStretchGraphic(ADestination: TBitmap; ASource: TGraphic;
  ...
  begin
    if ASmoothStretch and not cxSizeIsEqual(ADestRect, ASourceRect) then
    begin
...
procedure cxGetTextLines(const AText: string; ACanvas: TcxCanvas; const ARect: TRect; ALines: TStrings);
  ...
  procedure FindNextWordStart(const S: string; var ALineStart: Integer; var ANextWord: TcxWordPosition);
  begin
    while IsNewLineSymbol(S, ALineStart) do
      Inc(ALineStart);
    ANextWord.Start := ALineStart;
  end;

  function CheckNewLineSymbol(const S: string; var ANextWord: TcxWordPosition; var ALineStart: Integer): Boolean;
  var
    I: Integer;
  begin
    Result := False;
    for I := ALineStart to ANextWord.Finish do
      ...
      begin
        ALines.Add(Copy(S, ALineStart, I - ALineStart));
        ALineStart := I + 1;
        FindNextWordStart(S, ALineStart, ANextWord);
        Result := True;
        Break;
      end;
  end;
...
begin
  if IsGlyphAssigned(AGraphic) and (AGraphic is TdxCustomSmartImage) then
    ...
    Result := cxGraphicExtension(TGraphicClass(AGraphic));
end;
...
procedure TdxAdjustFontSizeCustomHelper.Calculate(AAvailableWidth: Integer; const AText: string);
var
...
begin
  ...
  try
    ...
    begin
      ASavedFontSize := Font.Size;
      ALastSuitableSize := -1;
      H := Font.Size;
      L := GetMinimalFontSize;
      while L <= H do
      begin
        Font.Size := (L + H) div 2;
        ATextWidth := GetTextWidth(AText);
      ...
      if ATextWidth > AAvailableWidth then
        CheckFontSize(ALastSuitableSize, ASavedFontSize, L);
    end;
  finally
    AfterCalculate;
  end;
...
procedure TdxAdjustFontSizeCustomHelper.CheckFontSize(ALastSuitableSize, AOriginalSize, AMinSize: Integer);
begin
  if ALastSuitableSize > 0 then
    Font.Size := ALastSuitableSize
  else
    Font.Size := AMinSize;
end;
...
function TdxAdjustFontSizeHelper.GetMinimalFontSize: Integer;
begin
  Result := 1;
end;
...
{ TdxImageListPaintCache.TImageListCacheHelper.TImageID }

constructor TdxImageListPaintCache.TImageListCacheHelper.TImageID.Create(AImageIndex: Integer; ADrawMode: TcxImageDrawMode;
  AUseLeftBottomPixelAsTransparent: Boolean; const APalette: IdxColorPalette; ASmoothImage: Boolean);
begin
  Index := AImageIndex;
  DrawMode := ADrawMode;
  ...
  if APalette <> nil then
    PaletteID := APalette.GetID
  else
    FillChar(PaletteID, SizeOf(PaletteID), 0);
end;

function TdxImageListPaintCache.TImageListCacheHelper.TImageID.IsEqual(const AID: TImageID): Boolean;
begin
  Result :=
    (Index = AID.Index) and
    (DrawMode = AID.DrawMode) and
    IsEqualGUID(PaletteID, AID.PaletteID) and
    (SmoothImage = AID.SmoothImage) and
    (UseLeftBottomPixelAsTransparent = AID.UseLeftBottomPixelAsTransparent);
end;

{ TdxImageListPaintCache.TImageListCacheHelper.TImages.TImageComparer }

function TdxImageListPaintCache.TImageListCacheHelper.TImages.TImageComparer.Equals(const Left, Right: TImageID): Boolean;
begin
  Result := Left.IsEqual(Right);
end;

function TdxImageListPaintCache.TImageListCacheHelper.TImages.TImageComparer.GetHashCode(const Value: TImageID): Integer;
begin
...
begin
  inherited Create([doOwnsValues], ACapacity, TImages.TImageComparer.Create);
end;
...
procedure TdxImageListPaintCache.TImageListCacheHelper.PrepareImage(AImage: TdxGPImage; const ASize: TSize;
  const AImageID: TImageID; APalette: IdxColorPalette);
begin
  TransferBitmap.RefreshImage(ASize.cx, ASize.cy);
  cxDrawImage(TransferBitmap.cxCanvas.Handle, TransferBitmap.ClientRect, TransferBitmap.ClientRect, nil, FImageList,
    AImageID.Index, AImageID.DrawMode, AImageID.SmoothImage, 0, clNone, AImageID.UseLeftBottomPixelAsTransparent, APalette);
  AImage.Assign(TransferBitmap);
end;

procedure TdxImageListPaintCache.TImageListCacheHelper.Draw(ACanvas: TcxCanvas; const ABounds: TRect; AIndex: Integer;
  ADrawMode: TcxImageDrawMode; AUseLeftBottomPixelAsTransparent: Boolean; const APalette: IdxColorPalette; ASmoothImage: Boolean);
var
  AImage: TdxGPImage;
  AImageID: TImageID;
  AImages: TImages;
...
begin
  ...
  AImageID.Create(AIndex, ADrawMode, AUseLeftBottomPixelAsTransparent, APalette, ASmoothImage);
  if not AImages.TryGetValue(AImageID, AImage) then
  begin
    AImage := TdxGPImage.Create;
    PrepareImage(AImage, ASize, AImageID, APalette);
    AImages.Add(AImageID, AImage);
  end;
  AImage.StretchDraw(ACanvas.Handle, ABounds);
end;
