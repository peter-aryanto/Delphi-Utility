...
begin
  ...
  Filer.DefineProperty('IsFontAssigned', ReadIsFontAssigned, WriteIsFontAssigned, IsFontStored);
end;
...
begin
  if (RootContainer = nil) or
    not RootContainer.InnerControlDefaultHandler(TMessage(Message)) then
...
procedure TcxCustomInnerListBox.SetExternalScrollBarsParameters;
  ...
  begin
    ...
    if cxGetScrollBarInfo(Handle, Integer(ScrollBarObjects[AScrollBar.Kind]), AScrollBarInfo) then
    begin
