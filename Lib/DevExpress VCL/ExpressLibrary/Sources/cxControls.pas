...
{$ENDIF}
  ...
  dxCore, dxCoreClasses, dxMessages, cxGraphics, cxScrollBar, cxGeometry,
  dxTouch, cxLookAndFeels, cxLookAndFeelPainters, dxForms, dxCoreGraphics, dxAnimation, cxClasses;
...
//    class procedure ScrollContent(AControl: IdxScrollingControl; APrevPos, ACurPos: Integer; AHorzScrolling: Boolean);
  ...
  public
    ...
    class function CalculateBounds(const AAreaBounds, AClientRect: TRect; APlace: TcxArrowPlace; AScaleFactor: TdxScaleFactor): TRect;
...
uses
  SysUtils, ActnList, Math, Registry, dxUxTheme, dxThemeConsts,
  cxLibraryConsts, cxLibraryStrs, cxDWMApi, cxFormats, dxThemeManager, dxDPIAwareUtils, dxGDIPlusClasses, dxHooks;
...
begin
  ...
  if (AActiveScrollUIOwner <> nil) and
    AActiveScrollUIOwner.HasVisibleUI then
  begin
    AOwnerControl := AActiveScrollUIOwner.GetOwnerControl;
    if (AOwnerControl <> nil) and AOwnerControl.HandleAllocated and
      (IsChild(AChangedWnd, AOwnerControl.Handle) or dxIsMDIChildWindow(AChangedWnd)) then
...
begin
  ...
  begin
    AOwnerControl := AActiveScrollUIOwner.GetOwnerControl;
    if (AOwnerControl <> nil) and AOwnerControl.HandleAllocated and
      ((AChangedWnd = AOwnerControl.Handle) or IsChild(AChangedWnd, AOwnerControl.Handle)) then
      DoHideTouchScrollUI(True);
  end;
end;
...
begin
  ...
  AOwnerControl := AScrollbarsOwner.GetOwnerControl;
  if (AOwnerControl <> nil) and AOwnerControl.HandleAllocated and
    (IsChild(AChangedWnd, AOwnerControl.Handle) or IsMDIForm(Application.MainForm)) then
...
procedure TcxControlPopupScrollBar.SetVisible(AValue: Boolean);
  ...
  begin
    dxSetZOrder(WindowHandle, AWndAfter, False, SWP_NOOWNERZORDER);
  end;
...
begin
  if (OwnerControl.GetScrollbarMode = sbmHybrid) and Visible and not AValue then
    GetHybridScrollbarOwner.GetManager.ResetSize;
...
begin
  ...
  begin
    AOwner := GetParent(SizeGrip.Handle);
    APrevWnd := GetNextWindow(AOwner, GW_HWNDPREV);
    if APrevWnd <> 0 then
      dxSetZOrder(SizeGrip.Handle, APrevWnd, False, SWP_NOOWNERZORDER);
  end;
end;
...
var
  APrevUpdatingScrollBars, APrevHScrollBarActive, APrevVScrollBarActive: Boolean;
  ...
  begin
    if IsScrollBarActive(AScrollBar.Kind) <> AInitialVisible then
    begin
  ...
  function IsScrollBarVisibilityChanged: Boolean;
  begin
    Result := HasScrollBarArea and
      ((IsScrollBarActive(sbHorizontal) <> APrevHScrollBarActive) or
      (IsScrollBarActive(sbVertical) <> APrevVScrollBarActive));
  end;

  procedure InternalScrollBarVisibilityChanged;
  var
    AChangedScrollBars: TScrollBarKinds;
  begin
    AChangedScrollBars := [];
    if IsScrollBarActive(sbHorizontal) <> APrevHScrollBarActive then
      AChangedScrollBars := AChangedScrollBars + [sbHorizontal];
    if IsScrollBarActive(sbVertical) <> APrevVScrollBarActive then
      AChangedScrollBars := AChangedScrollBars + [sbVertical];
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        CheckScrollBarParams(HScrollBar, FInitialHScrollBarActive);
        CheckScrollBarParams(VScrollBar, FInitialVScrollBarActive);
      end
      else
        MainScrollBars.ApplyData;
    end;
    if IsScrollBarVisibilityChanged then
      InternalScrollBarVisibilityChanged
    else
...
procedure TcxDragAndDropArrow.Init(const AOwnerOrigin: TPoint; const AAreaBounds, AClientRect: TRect; APlace: TcxArrowPlace);
  ...
  begin
    Result := CalculateBounds(AAreaBounds, AClientRect, APlace, AScaleFactor);
...
begin
  ...
  begin
    ...
    if (ACWPStruct.message = WM_ENABLE) and (ACWPStruct.lParam = 0) or
      ...
      ((Windows.PWindowPos(ACWPStruct.lParam).flags and SWP_HIDEWINDOW) <> 0) or (ACWPStruct.message = WM_CLOSE) then
    begin
