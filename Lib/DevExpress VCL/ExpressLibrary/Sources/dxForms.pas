...
type
  ...
  public
    ...
    class procedure ScaleForPPI(Form: TCustomForm; TargetDPI: Integer; WindowRect: PRect = nil);
  end;
...
uses
  dxDPIAwareUtils, cxControls, dxHooks, dxCoreClasses, TypInfo;
...
begin
  ...
  {$ENDIF}
    if AParentFont and (Frame.Parent <> nil) then
    begin
...
begin
  TdxFormHelper.ScaleForPPI(TCustomFormAccess(Self), ATargetDPI, AWindowRect);
end;
...
begin
  if Scaled then
    ScaleForPPI(LoWord(Message.WParam), PRect(Message.LParam));
end;
...
begin
  if dxSystemInfo.IsRemoteSession then 
    Result := dxSetWindowStyle(AWndHandle, 0, TdxSetOperation.soAdd)
  else
...
class procedure TdxFormHelper.ScaleForPPI(Form: TCustomForm; TargetDPI: Integer; WindowRect: PRect);
