...
uses
  SysUtils, dxCore, StrUtils, Math, Character, dxDPIAwareUtils;
...
begin
  ...
  try
    if fsItalic in AStyles then
      AResult.Append('italic').Append(' ');
    if fsBold in AStyles then
      AResult.Append('bold').Append(' ');
...
begin
  if cxPointIsEqual(StartPoint, FinishPoint) then
    Exit;
  if cxPointIsEqual(Radius, dxNullPointF) then
  begin
