...
{$ENDIF}
  ...
  cxContainer, dxSkinsLookAndFeelPainter, dxSkinsCore, cxScrollBar, cxLookAndFeels, dxUxTheme, dxSkinInfo, dxForms;
...
procedure TdxSkinFormScrollBarsController.CalculateScrollArea;
  ...
  procedure CalculateScrollBarBounds(AKind: TScrollBarKind);
  begin
    ScrollBarViewInfo[AKind].FInfo := DoGetScrollBarInfo(AKind);
    ScrollBarViewInfo[AKind].FVisible := IsScrollBarVisible(AKind);
    ScrollBarViewInfo[AKind].FBounds := GetScrollBarBounds(ViewInfo.ClientBounds, AKind);
  end;

begin
  CalculateScrollBarBounds(sbHorizontal);
  CalculateScrollBarBounds(sbVertical);
  if SizeGripVisible then
  begin
...
function TdxSkinFormScrollBarsController.GetScrollBarViewInfo(
  AKind: TScrollBarKind): TdxSkinFormScrollBarViewInfo;
begin
...
function My_SetScrollPos(hWnd: HWND; nBar, nPos: Integer; bRedraw: BOOL): Integer; stdcall;
begin
  bRedraw := bRedraw and not TdxSkinWinController.IsSkinActive(hWnd);
  Result := SetScrollPos(hWnd, nBar, nPos, bRedraw);
end;

function My_SetScrollInfo(hWnd: HWND; BarFlag: Integer; const ScrollInfo: TScrollInfo; Redraw: BOOL): Integer; stdcall;
begin
