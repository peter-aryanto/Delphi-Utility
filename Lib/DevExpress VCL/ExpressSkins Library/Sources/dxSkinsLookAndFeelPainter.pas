...
type
  ...
  public
    ...
    procedure DrawRangeTrackBarScaledRightThumb(ACanvas: TcxCanvas; const ARect: TRect; AState: TcxButtonState;
      AHorizontal: Boolean; ATicks: TcxTrackBarTicksAlign; AThumbColor: TColor; AScaleFactor: TdxScaleFactor); override;
    // Splitter
    procedure DrawScaledSplitter(ACanvas: TcxCanvas; const ARect: TRect; AHighlighted, AClicked, AHorizontal: Boolean; AScaleFactor: TdxScaleFactor); override;
...
begin
  if not DrawSkinElement(SkinInfo.ExpandButton, ACanvas, R, AScaleFactor, StateMap[AState], Byte(AExpanded)) then
...
begin
  if not DrawSkinElement(SkinInfo.ExpandButton, ACanvas, R, AScaleFactor, esNormal, Byte(AExpanded)) then
...
begin
  ...
  begin
    ...
    AScrollInfoElement.Element.Draw(ACanvas.Handle, R, AScaleFactor);
  end;
end;
...
begin
  ...
  if (AScrollPartInfo = nil) or (AScrollPartInfo.Element = nil) then
    ...
    if not ((APart in [sbpPageUp, sbpPageDown]) and (AState = cxbsNormal)) then
      AScrollPartInfo.DrawScaled(ACanvas.Handle, R, ButtonStateToSkinState[AState], AScaleFactor);
end;
...
begin
  if not DrawSkinElement(SkinInfo.SizeGrip, ACanvas, ARect) then
...
begin
  if not DrawSkinElement(SkinInfo.PopupMenuSideStrip, ACanvas, ARect, AScaleFactor) then
...
procedure TdxSkinLookAndFeelPainter.DrawSpreadSheetScaledGroupExpandButtonGlyph(ACanvas: TcxCanvas;
  const R: TRect; AState: TcxButtonState; AExpanded: Boolean; AScaleFactor: TdxScaleFactor; ADefaultGlyphs: TCustomImageList = nil);
begin
  if SkinInfo.NavigatorGlyphs <> nil then
    SkinInfo.NavigatorGlyphs.Draw(ACanvas.Handle, cxRectCenter(R, NavigatorScaledButtonGlyphSize(AScaleFactor)), IfThen(AExpanded, 8, 6))
  else
  ...
  begin
    ...
    try
      ...
      SkinInfo.FooterPanel.Draw(ACanvas.Handle,
        cxRectInflate(ARect, FooterBorderSize, FooterSeparatorSize, FooterBorderSize, FooterBorderSize));
    finally
      ACanvas.RestoreClipRegion;
    end;
  end;
...
procedure TdxSkinLookAndFeelPainter.DrawPanelContent(ACanvas: TcxCanvas; const ABorderRect: TRect; ABorder: Boolean);
begin
  if SkinInfo.GroupBoxClient <> nil then
  begin
    SkinInfo.GroupBoxClient.Draw(ACanvas.Handle, ABorderRect);
    if ABorder then
...
procedure TdxSkinLookAndFeelPainter.DrawScaledSplitter(ACanvas: TcxCanvas;
  const ARect: TRect; AHighlighted, AClicked, AHorizontal: Boolean; AScaleFactor: TdxScaleFactor);
const
  StateMap: array[Boolean] of TdxSkinElementState = (esNormal, esHot);
begin
  if not DrawSkinElement(SkinInfo.Splitter[AHorizontal], ACanvas, ARect, AScaleFactor, StateMap[AHighlighted]) then
    inherited DrawScaledSplitter(ACanvas, ARect, AHighlighted, AClicked, AHorizontal, AScaleFactor);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        try
          SkinInfo.IndicatorImages.Draw(ABitmap.Canvas.Handle, ABitmap.ClientRect, AScaleFactor, AIndex);
        ...
        SkinInfo.IndicatorImages.Draw(ACanvas.Handle, R, AScaleFactor, AIndex);
    end;
  end;
