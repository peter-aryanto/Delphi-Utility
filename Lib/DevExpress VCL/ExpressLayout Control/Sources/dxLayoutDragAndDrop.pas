    ...
    case ADropAreaInfo.DropAreaPart of
      ...
      apNewLayout:
        if (AParentGroup.LayoutDirection = ldHorizontal) and (ADropAreaInfo.DestinationItem.AlignHorz in [ahParentManaged, ahLeft]) or
          ...
          Result := 0;
      apLastChild:
        if (AParentGroup.LayoutDirection = ldHorizontal) and (ADropAreaInfo.ExpectedAlign.Horz = ahRight) or
           (AParentGroup.LayoutDirection = ldVertical) and (ADropAreaInfo.ExpectedAlign.Vert = avBottom) then
          Result := 0
        else
          Result := AParentGroup.Count;
    else
...
begin
  ...
  if TdxLayoutGroupAccess(ADragDropInfo.DestinationGroup).IsSuperfluous(True, False) then
...
begin
  ...
  case FDragDropInfo.ExpectedAlign.Horz of
...
begin
  if TdxLayoutContainerAccess(SourceItem.Container).IsFloatingSupported then
