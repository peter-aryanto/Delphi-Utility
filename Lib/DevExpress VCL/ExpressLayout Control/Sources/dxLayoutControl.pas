...
begin
  if not IsDestroying and
    ((Container.Root.AlignHorz in [ahClient, ahParentManaged]) and (sbVertical in AScrollBars) or
     (Container.Root.AlignVert in [avClient, avParentManaged]) and (sbHorizontal in AScrollBars)) then
    inherited
  else
