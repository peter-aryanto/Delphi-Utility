...
type
  ...
  TdxLayoutItemCanResizeEvent = procedure(Sender: TObject; AItem1, AItem2: TdxCustomLayoutItem; const ANewSize1, ANewSize2: Integer; var ADelta: Integer; var AAccept: Boolean) of object;
  ...
  protected
    ...
    procedure DoPack(AKeepAlign: Boolean); virtual;
...
//  published
  ...
  private
    ...
    procedure SetAutoCreatedForWrap(Value: Boolean);
    ...
    procedure WriteAutoCreated(Writer: TWriter);
    procedure ReadAutoCreatedForWrap(Reader: TReader);
    procedure WriteAutoCreatedForWrap(Writer: TWriter);
  ...
  protected
    ...
    procedure ContainerChanged; override;
    procedure DoGetTabOrderList(List: TList); override;
    procedure DoChildPack(AKeepAlign: Boolean);
    procedure DoPack(AKeepAlign: Boolean); override;
    ...
    function ProcessDialogKey(ACharCode: Word; AKeyData: Integer;
      AFocusedItem: TdxCustomLayoutItem): Boolean; override;
    procedure RestoreItemControlBounds; override;
    ...
    function IsSuperfluous(AKeepAlign, AForDestroy: Boolean): Boolean;
    ...
    property AutoCreatedForWrap: Boolean read FAutoCreatedForWrap write SetAutoCreatedForWrap;
...
//  published
  ...
  TdxLayoutAutoCreatedGroup = class(TdxCustomLayoutGroup)
  protected
    procedure Loaded; override;
    class function GetItemClassKind: Integer; override;
  public
  ...
  protected
    ...
    // Wrap functions
    function AddWrapLevel(AGroup: TdxCustomLayoutGroup; ALayoutDirection: TdxLayoutDirection): TdxLayoutAutoCreatedGroup;
    function SimpleWrap(AGroup: TdxCustomLayoutGroup; AMaxColumnCount, AMaxWidth: Integer): Boolean;
    procedure SqueezeWrappingGroups;
    procedure PopulateWrappingGroups;
    procedure UnclenchWrappingGroups;
  ...
  protected
    ...
    function GetLevelingItemCaptionWidth(AItem: TdxLayoutControlItemViewInfo; AMaxCaptionSize: Integer): Integer; virtual;
  ...
  public
    ...
    property ItemsViewInfo: TdxLayoutGroupViewInfo read GetItemsViewInfo; // obsolette
    property LayoutLookAndFeel: TdxCustomLayoutLookAndFeel read GetLayoutLookAndFeel;
    property Offset: TPoint read FOffset write SetOffset;
...
function dxIsValueInDiapasone(AValue, ABound1, ABound2: Integer; AIncludeBounds: Boolean = True): Boolean;
begin
...
procedure TdxCustomLayoutGroup.SetAutoCreatedForWrap(Value: Boolean);
begin
  dxTestCheck(FAutoCreated and Value, 'SetAutoCreatedForWrap fails');
  FAutoCreatedForWrap := Value;
end;
...
procedure TdxCustomLayoutGroup.WriteAutoCreated(Writer: TWriter);
begin
  Writer.WriteBoolean(FAutoCreated);
end;

procedure TdxCustomLayoutGroup.ReadAutoCreatedForWrap(Reader: TReader);
begin
  FAutoCreatedForWrap := Reader.ReadBoolean;
end;

procedure TdxCustomLayoutGroup.WriteAutoCreatedForWrap(Writer: TWriter);
begin
  Writer.WriteBoolean(FAutoCreatedForWrap);
end;
...
procedure TdxCustomLayoutGroup.DoChildPack(AKeepAlign: Boolean);
var
  I: Integer;
begin
  for I := Count - 1 downto 0 do
    Items[I].DoPack(AKeepAlign);
end;

procedure TdxCustomLayoutGroup.DoPack(AKeepAlign: Boolean);
var
  I: Integer;
begin
  DoChildPack(AKeepAlign);
  if IsSuperfluous(AKeepAlign, True) then
  begin
    if AKeepAlign then
      for I := 0 to Count - 1 do
    ...
    MoveChildrenToParent(AKeepAlign);
...
procedure TdxCustomLayoutGroup.DefineProperties(Filer: TFiler);
  ...
  function IsAutocreatedStored: Boolean;
  begin
    Result := (Filer.Ancestor = nil) and AutoCreated or
      (Filer.Ancestor <> nil) and (TdxCustomLayoutGroup(Filer.Ancestor).AutoCreated <> AutoCreated);
  end;

  function IsAutoCreatedForWrapStored: Boolean;
  begin
    Result := (Filer.Ancestor = nil) and FAutoCreatedForWrap or
      (Filer.Ancestor <> nil) and (TdxCustomLayoutGroup(Filer.Ancestor).FAutoCreatedForWrap <> FAutoCreatedForWrap);
  end;

begin
  inherited;
  Filer.DefineProperty('Special', ReadSpecial, WriteSpecial, IsSpecialStored);
  Filer.DefineProperty('AutoCreated', ReadAutoCreated, WriteAutoCreated, IsAutocreatedStored);
  Filer.DefineProperty('AutoCreatedForWrap', ReadAutoCreatedForWrap, WriteAutoCreatedForWrap, IsAutoCreatedForWrapStored);
end;

function TdxCustomLayoutGroup.GetBaseName: string;
begin
...
procedure TdxCustomLayoutGroup.BuildVisibleItemsList;
var
  I: Integer;
begin
  FVisibleItems.Clear;
...
begin
  ...
  ACounter := 0;
  for I := 0 to AGroup.Count - 1 do
    if AGroup.Items[I].IsGroup and not AGroup.Items[I].AsGroup.FAutoCreatedForWrap then
      ACounter := 0
    else
    begin
      Inc(ACounter);
...
function TdxCustomLayoutGroup.CanWrapItems: Boolean;
  ...
  function GetStableParent(AGroup: TdxCustomLayoutGroup): TdxCustomLayoutGroup;
  begin
    Result := AGroup.Parent;
    if Result.AutoCreatedForWrap then
      Result := GetStableParent(Result);
  end;

begin
  Result := not IsFloatingRoot and (WrapItemsMode <> wmNone) and
    (CanWrapImmediateChildItems or FAutoCreatedForWrap or (Parent <> nil) and Parent.CanWrapAllChildItems) and
      not IsAvailable and ((RealAlign.Horz = ahClient) or IsRoot or GetStableParent(Self).IsRoot);
end;

function TdxCustomLayoutGroup.CanWrapImmediateChildItems: Boolean;
begin
  Result := (WrapItemsMode in [wmImmediateChildren, wmAllChildren]) or
...
function TdxCustomLayoutGroup.IsSuperfluous(AKeepAlign, AForDestroy: Boolean): Boolean;
...
begin
  if AForDestroy and IsTakeoffParent or FIsFloat or IsDragged then
    Result := False
  else
  begin
    if AKeepAlign then
    begin
      Result := not IsRoot and not Locked and FAutoCreated and not IsSignificantOptionAssigned and
        ((Count = 0) or (Count = 1) and IsChildAlignSuperfluous(Items[0]) or (Parent <> nil) and (Parent.Count = 1) and dxLayoutIsSameAlign(TdxLayoutAlign(Self.RealAlign), TdxLayoutAlign(Parent.RealAlign)));

      Result := Result or (Parent <> nil) and FAutoCreated and
        ((Parent.LayoutDirection = LayoutDirection) or (Parent.Count = 1) or (Count = 1)) and (IsAlignPushInside or IsAlignSuperfluous);
    end
    else
      Result := AutoCreated and ((Parent = nil) or (Count = 0) or (Parent <> nil) and
        (Parent.AutoCreatedForDirection and (Parent.LayoutDirection = LayoutDirection) or 
          not Parent.AutoCreatedForWrap and (Parent.LayoutDirection = LayoutDirection) and (AlignHorz = ahParentManaged) or 
          not Parent.AutoCreated and (Count = 1) or 
          (Parent.AutoCreatedForDirection or Parent.AutoCreatedForWrap and AutoCreatedForWrap) and (Parent.Count = 1) or 
          (Count = 1) and dxLayoutIsSameAlign(Items[0].Align, Align)));
  end;
end;
...
constructor TdxLayoutAutoCreatedGroup.Create(AOwner: TComponent);
begin
  inherited;
  FHidden := True;
  FAutoCreated := True;
...
class function TdxLayoutAutoCreatedGroup.GetItemClassKind: Integer;
begin
...
begin
  ...
  begin
    FControlAdapter.SetControlAutoWidth((RealAlign.Horz <> ahClient) or (ControlOptions.AlignHorz <> ahClient));
    FControlAdapter.SetControlAutoHeight((RealAlign.Vert <> avClient) or (ControlOptions.AlignVert <> avClient));
  end;
end;
...
procedure TdxCustomLayoutItem.DoPack(AKeepAlign: Boolean);
begin
// do nothing
end;
...
begin
  Result := (FVisible or IsDesigning or IsDragged) and ((Parent = nil) or Parent.AllowShowChild(Self));
end;
...
begin
  Result :=
    ...
    (VisibleIndex < 1) or
    Parent.VisibleItems[VisibleIndex-1].IsHorzPositionStable and (Parent.VisibleItems[VisibleIndex-1].RealAlign.Horz in [ahLeft, ahRight]);
end;
...
begin
  Result :=
    ...
    (VisibleIndex < 1) or
    Parent.VisibleItems[VisibleIndex-1].IsVertPositionStable and (Parent.VisibleItems[VisibleIndex-1].RealAlign.Vert in [avTop, avBottom]);
end;

function TdxCustomLayoutItem.IsHorzPositionStable: Boolean;
begin
  Result := IsRoot or (Parent.IsHorzPositionStable or (Parent.ViewInfo.GetRealItemControlAreaAlignment = catOwn)) and
    IsHorzLocalPositionStable;
end;

function TdxCustomLayoutItem.IsVertPositionStable: Boolean;
begin
  Result := IsRoot or (Parent.IsVertPositionStable or (Parent.ViewInfo.GetRealItemControlAreaAlignment = catOwn)) and
    IsVertLocalPositionStable;
end;
...
begin
  ...
  if ASource is TdxCustomLayoutItemCaptionViewInfo then
  begin
...
begin
  dxTestCheck(ViewData <> nil, 'TdxCustomLayoutItemViewInfo.Destroy fails');
...
begin
  ...
  begin
    ParentViewInfo.ViewData.ItemIndex := Item.Index;
    ParentViewInfo.MakeVisible(ARect, AFully);
  end;
end;
...
var
  ...
  begin
    ...
    begin
//      GroupViewInfo.Group.FScrollPos.X := 0;
      if GroupViewInfo.FMajorScrollBar <> nil then 
      begin
        GroupViewInfo.ContainerViewInfo.FNeedRecalculate := True;
...

{
            ...
            begin
              if (FItemInfos[I].ViewInfo is TdxLayoutGroupViewInfo) and (TdxLayoutGroupViewInfo(FItemInfos[I].ViewInfo).Group.FAllowScroll) then
              TdxLayoutGroupViewInfo(FItemInfos[I].ViewInfo).CreateScrollBar;
            end
            else
            }
...
begin
  AMaxMinorSize := 0;
  for I := 0 to ItemViewInfoCount - 1 do
//    AMaxMinorSize := Max(AMaxMinorSize, FItemInfos[I].MinMinorSize);
    AMaxMinorSize := Max(AMaxMinorSize, FItemInfos[I].OriginalMinorSize);
//    AMaxMinorSize := Max(AMaxMinorSize, FItemInfos[I].SufficientMinorSize);

  for I := 0 to ItemViewInfoCount - 1 do
  begin
    AMinorSize := FItemInfos[I].OriginalMinorSize;
//    AMinorSize := FItemInfos[I].SufficientMinorSize;
    R := FItemInfos[I].AbsBounds;
    case GetMinorAlign(FItemInfos[I].ViewInfo.Align) of
      aaNear:
        begin
      ...
      aaCenter:
        begin
          R.NearMinor := (AItemsAreaBounds.NearMinor + AItemsAreaBounds.FarMinor - AMinorSize) div 2 - GroupViewInfo.GetScrollPos.Minor;
          R.FarMinor := R.NearMinor + AMinorSize;
        end;
      aaFar:
        begin
  ...
  begin
//    GroupViewInfo.Group.FScrollPos.Y := 0;
    if GroupViewInfo.FMinorScrollBar <> nil then 
    begin
      GroupViewInfo.ContainerViewInfo.FNeedRecalculate := True;
...
begin
  ...
  begin
    if AAbsRect.NearMajor < AAbsItemsAreaBounds.NearMajor then
      Result := Result - (AAbsItemsAreaBounds.NearMajor - AAbsRect.NearMajor)
    else
      if AAbsRect.FarMajor > AAbsItemsAreaBounds.FarMajor then
        Result := Result + Min(AAbsRect.FarMajor - AAbsItemsAreaBounds.FarMajor, AAbsRect.NearMajor - AAbsItemsAreaBounds.NearMajor);
  end
  else
    if AAbsRect.NearMajor > AAbsItemsAreaBounds.FarMajor then
      Result := Result + (AAbsRect.FarMajor - AAbsItemsAreaBounds.FarMajor)
    else
      if AAbsRect.FarMajor < AAbsItemsAreaBounds.FarMajor then
        Result := Result - (AAbsItemsAreaBounds.FarMajor - AAbsRect.FarMajor);
end;
...
begin
  ...
  begin
    if AAbsRect.NearMinor < AAbsItemsAreaBounds.NearMinor then
      Result := Result - (AAbsItemsAreaBounds.NearMinor - AAbsRect.NearMinor)
    else
      if AAbsRect.FarMinor > AAbsItemsAreaBounds.FarMinor then
        Result := Result + Min(AAbsRect.FarMinor - AAbsItemsAreaBounds.FarMinor, AAbsRect.NearMinor - AAbsItemsAreaBounds.NearMinor);
  end
  else
    if AAbsRect.NearMinor > AAbsItemsAreaBounds.FarMinor then
      Result := Result + (AAbsRect.FarMinor - AAbsItemsAreaBounds.FarMinor)
    else
...
begin
  ...
  // Proper Align
  if ((ADropAreaInfo.DropAreaPart in [apTop, apBottom]) and (LayoutDirection = ldHorizontal) or
      (ADropAreaInfo.DropAreaPart in [apLeft, apRight]) and (LayoutDirection = ldVertical)) and
     (AAbsItemBounds.Size.Major > dxLayoutItemControlDefaultMinSize) then
  begin
    ...
    if (GetMajorAlign(ADropAreaInfo.DestinationItem.RealAlign) = aaClient) and
       ...
       (dxIsValueInDiapasone(GetMajorValue(ADropAreaInfo.DropClientPoint), AAbsCenter.NearMajor, AAbsCenter.FarMajor)) then
      SetMajorAlign(Result, aaClient)
    else
  ...
  // Opposite Align
  if ((ADropAreaInfo.DropAreaPart in [apLeft, apRight]) and (LayoutDirection = ldHorizontal) or
      (ADropAreaInfo.DropAreaPart in [apTop, apBottom]) and (LayoutDirection = ldVertical)) and
     (GetAbsRect(GroupViewInfo.ItemsAreaBounds).Size.Minor = AAbsItemBounds.Size.Minor) then
  begin
    ...
    if (GetMinorAlign(ADropAreaInfo.DestinationItem.RealAlign) = aaClient) and
       ...
       dxIsValueInDiapasone(GetMinorValue(ADropAreaInfo.DropClientPoint), AAbsCenter.NearMinor, AAbsCenter.FarMinor) then
      SetMinorAlign(Result, aaClient)
    else
...
function TdxLayoutGroupViewInfoSpecific.GetDropRectInsideRect(const ARect: TRect; const ADropAreaInfo: TdxLayoutDragDropInfo): TRect;
  ...
  begin
    ...
    case ADropAreaInfo.ExpectedAlign.Horz of
...
begin
  ...
  GroupViewInfo.SetScrollBounds(GroupViewInfo.FMajorScrollBar, R);
end;
...
begin
  ...
  GroupViewInfo.SetScrollBounds(GroupViewInfo.FMinorScrollBar, R);
end;
...
begin
  ...
  for I := 0 to GroupViewInfo.ItemViewInfoCount - 1 do
  begin
    if GroupViewInfo.ItemViewInfos[I].Item.RealAlign.Horz = ahLeft then
      Result.Left := Max(GroupViewInfo.ItemViewInfos[I].Bounds.Right, Result.Left);
    if GroupViewInfo.ItemViewInfos[I].Item.RealAlign.Horz = ahRight then
      Result.Right := Min(GroupViewInfo.ItemViewInfos[I].Bounds.Left, Result.Right);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if not IsPopupScrollBars then
        Inc(Result, cxRectHeight(GroupViewInfo.FMajorScrollBar.BoundsRect));
  ...
  begin
    ...
    begin
      if not IsPopupScrollBars then
        Inc(Result, cxRectWidth(GroupViewInfo.FMinorScrollBar.BoundsRect));
...
begin
  ...
  begin
    ...
    begin
      if not IsPopupScrollBars then
        Inc(Result, cxRectWidth(GroupViewInfo.FMajorScrollBar.BoundsRect));
      Inc(Result, GroupViewInfo.LayoutLookAndFeel.GetGroupBorderOffset(Container, sdRight, GroupViewInfo.CaptionSide));
    end;
  end;
  ...
  begin
    ...
    begin
      if not IsPopupScrollBars then
        Inc(Result, cxRectHeight(GroupViewInfo.FMinorScrollBar.BoundsRect));
      Inc(Result, GroupViewInfo.LayoutLookAndFeel.GetGroupBorderOffset(Container, sdBottom, GroupViewInfo.CaptionSide));
    end;
  end;
end;
...
begin
  inherited;
  FTabSheetContenOffset.X := GroupViewInfo.LayoutLookAndFeel.GetTabSheetContentOffsetHorz(Container);
  FTabSheetContenOffset.Y := GroupViewInfo.LayoutLookAndFeel.GetTabSheetContentOffsetVert(Container);
end;
...
begin
  ...
  begin
    ...
    if (TabbedOptions.TabPosition in [tpTop, tpBottom]) and (AClientPoint.X > C.X) or
       (TabbedOptions.TabPosition in [tpLeft, tpRight]) and (AClientPoint.Y < C.Y) then
      AAreaPart := apAfter
    else
      AAreaPart := apBefore;
  end
...
begin
  FTabControlBounds := cxRectOffset(FTabControlBounds, ADiff);
  CalculateTabControl;
end;
...
begin
  ...
  begin
    ...
    case ASide of
      sdLeft, sdRight:
        Inc(Result, FTabSheetContenOffset.X); 
      sdTop, sdBottom:
        Inc(Result, FTabSheetContenOffset.Y); 
    end;
    case ASide of
      sdLeft:
...
begin
  ...
  if Group.CanWrapItems or Group.FAutoCreatedForWrap then
    if Group.FAutoCreatedForWrap then
      Result := catAuto
    else
...
begin
  ...
  if FMajorScrollBar <> nil then
  begin
    SetScrollBounds(FMajorScrollBar, cxRectOffset(FMajorScrollBar.BoundsRect, ADiff));
    FMajorScrollBarBounds := FMajorScrollBar.BoundsRect;
    FMajorScrollBar.Visible := IsScrollBarsVisible;
  end;
  ...
  if FMinorScrollBar <> nil then
  begin
    SetScrollBounds(FMinorScrollBar, cxRectOffset(FMinorScrollBar.BoundsRect, ADiff));
    FMinorScrollBarBounds := FMinorScrollBar.BoundsRect;
    FMinorScrollBar.Visible := IsScrollBarsVisible;
  end;
end;
...
function TdxLayoutGroupViewInfo.GetLevelingItemCaptionWidth(AItem: TdxLayoutControlItemViewInfo; AMaxCaptionSize: Integer): Integer;
var
...
begin
  ...
  begin
    ...
    ACaptionDelta := AMaxCaptionSize - AItem.CaptionViewInfo.Width;
...
begin
  if not Container.IsCustomization then
    if Container.AllowWrapItems then
    begin
      Container.UnclenchWrappingGroups;
      Container.SqueezeWrappingGroups; 
    end
    else
      Container.UnclenchWrappingGroups;
  if AHard then
...
procedure TdxCustomLayoutItemCaptionPainter.DoDrawText(ACanvas: TcxCanvas);
  ...
  begin
    ...
    try
      ...
      begin
        ALineHeight := cxDrawText(ACanvas.Handle, ALines[I], ALineRect, AFormat, -1, ATextColor);
...
procedure TdxLayoutContainer.GetTabOrderList(List: TList);
  ...
  begin
    ...
    try
      for I := AList.Count - 1 downto 0 do
        if AList[I] <> ItemsParentControl then
        begin
...
begin
  Result := ViewInfo.IsMouseWheelHandleNeeded(Shift, WheelDelta, MousePos);
end;
...
function IsItemHigest(AItem: TdxCustomLayoutItem): Boolean;
var
  I: Integer;
begin
  Result := True;
...
function TdxLayoutContainer.AddWrapLevel(AGroup: TdxCustomLayoutGroup; ALayoutDirection: TdxLayoutDirection): TdxLayoutAutoCreatedGroup;
begin
  Result := GetAutoCreatedGroup;
  Result.Align := dxLayoutAlign(ahClient, avParentManaged);
  Result.Parent := AGroup;
  Result.LayoutDirection := ALayoutDirection;
  Result.AutoCreatedForWrap := True;
end;

function TdxLayoutContainer.SimpleWrap(AGroup: TdxCustomLayoutGroup; AMaxColumnCount, AMaxWidth: Integer): Boolean;
  ...
  function IsItemMostHigest(AItem: TdxCustomLayoutItem): Boolean;
  var
    I: Integer;
  begin
    Result := True;
  ...
  begin
    ...
    begin
      ...
      if ANewWidth > AMaxWidth then
        Break;
      if not IsItemHigest(AGroup[I - 1]) and (GetHeight(AGroup[I - 1]) + GetHeight(AGroup[I]) < AGroupHeight) then
      begin
        ...
        AGroup[I-1].Move(ALevel, ALevel.Count);
        ALevel.AlignHorz := GetLevelAlign(ALevel);
        Result := True;
        Break;
      end
      else
  ...
  begin
    while ((GetWidth(AGroup) > AMaxWidth) or (AGroup.Count > AMaxColumnCount)) and SimpleOptimize(AGroup, AMaxWidth, AHeightInc) do
      {loop};
  end;
{
  procedure OptimizeWrapX(AGroup: TdxCustomLayoutGroup);
  var
    APrevGroup: TdxCustomLayoutItem;
    ALastItem: TdxCustomLayoutItem;
    ALevel1, ALevel2: TdxCustomLayoutGroup;
    AWidth: Integer;
  begin
    if AGroup.Index > 0 then
    begin
      ALastItem := AGroup[AGroup.Count - 1];
      if IsItemMostHigest(ALastItem) then
      begin
        APrevGroup := AGroup.Parent[AGroup.Index - 1];
        AWidth := GetWidth(ALastItem) + GetItemOffset(AGroup);
        if (GetWidth(AGroup) - AWidth < AMaxWidth) and (GetWidth(APrevGroup) + AWidth < AMaxWidth) then
        begin
          ALevel1 := AddWrapLevel(AGroup.Parent, ldVertical);
          ALevel1.Index := APrevGroup.Index;

          AGroup.Move(ALevel1, 0);
          APrevGroup.Move(ALevel1, 0);

          ALevel2 := AddWrapLevel(ALevel1.Parent, ldHorizontal);
          ALevel2.Index := ALevel1.Index;
          ALevel1.Move(ALevel2, 0);
          ALastItem.Move(ALevel2, 1);
        end;
      end;
    end;
  end;
...
begin
  ...
  if (AGroup.Count > 1) and (AMaxColumnCount > 0) then
  begin
    if (AMaxColumnCount > 1) and (AGroup.LayoutDirection = ldVertical) then
    begin
      Result := True;
      ...
      begin
        ...
        if AllowOptimizeWrappedItems and IsItemMostHigest(ACurrentItem) and (ACurrentLevel.Index > 0) and (GetHeight(AGroup[ACurrentLevel.Index-1]) * 1.5 < GetHeight(ACurrentItem)) then
        begin
        ...
        if (GetWidth(ACurrentLevel) > AMaxWidth) or (ACurrentLevel.Count > AMaxColumnCount) then
        begin
          ...
          begin
            ...
            if (GetWidth(ACurrentLevel) > AMaxWidth) and (IsItemHigest(ACurrentLevel[ACurrentLevel.Count - 1])) then  // rollback
            begin
              I := 0;
              while I < ACurrentLevel.Count do
              begin
                if ACurrentLevel.Items[0].IsGroup and ACurrentLevel.Items[0].AsGroup.FAutoCreatedForWrap and (ACurrentLevel.Items[0].AsGroup.FWrappingState = wsPopulated) then
...
procedure TdxLayoutContainer.SqueezeWrappingGroups;

  procedure SqueezeWrappingGroup(AGroup: TdxCustomLayoutGroup);
  var
    I: Integer;
    ALevel: TdxLayoutAutoCreatedGroup;
  begin
    ...
    begin
      ...
      begin
        ...
        if (AGroup.LayoutDirection = ldHorizontal) and not AGroup.FAutoCreated then
        begin
          ALevel := AddWrapLevel(AGroup, ldVertical);
...
procedure TdxLayoutContainer.PopulateWrappingGroups;
  ...
  function GetFreeSpace(AGroup: TdxCustomLayoutGroup): Integer;
  begin
    if AGroup.IsRoot then
      Result := Max(0, cxRectWidth(ClientBounds) - cxRectWidth(Root.ViewInfo.Bounds))
    else
    begin
      if AGroup.RealAlign.Horz <> ahClient then
        Result := cxRectWidth(AGroup.Parent.ViewInfo.ItemsAreaBounds) - GetGroupContentWidth(AGroup.Parent)
      else
        Result := 0;
      Result := Result + GetFreeSpace(AGroup.Parent);
    end;
  end;

  function GetMaxWidth(AGroup: TdxCustomLayoutGroup): Integer;
  begin
    Result := cxRectWidth(AGroup.ViewInfo.ItemsAreaBounds) + GetFreeSpace(AGroup);
  end;

  procedure PopulateWrappingGroup(AGroup: TdxCustomLayoutGroup);
  var
    I: Integer;
  begin
    if AGroup.GetVisible and (AGroup.FWrappingState <> wsPopulated) then
    begin
      if AGroup.CanWrapItems then
      begin
        if SimpleWrap(AGroup, MaxInt, GetMaxWidth(AGroup)) then
          if Root.RealAlign.Horz = ahClient then
            MediumChanged(AGroup)
          else
            MediumChanged(Root);
      end;
      ...
      for I := 0 to AGroup.Count - 1 do
        if AGroup[I].IsGroup then
          PopulateWrappingGroup(AGroup[I].AsGroup);

      if AGroup.AutoCreatedForWrap then
        AGroup.AlignHorz := GetLevelAlign(AGroup);
...
begin
  PopulateWrappingGroup(Root);
  Root.DoChildPack(False);
end;

procedure TdxLayoutContainer.UnclenchWrappingGroups;
  ...
  begin
    ...
    begin
      if AGroup[I].IsGroup then
        UnclenchWrappingGroup(AGroup[I].AsGroup);
      if AGroup[I].IsGroup and AGroup[I].AsGroup.AutoCreatedForWrap {and not AGroup[I].IsDragged} then
        AGroup[I].AsGroup.Dismiss
      else
        Inc(I);
    end;
  end;
...
initialization
  RegisterClasses([
    ...
    TdxLayoutSplitterItem, TdxLayoutImageItem, TdxLayoutGroup, TdxLayoutAutoCreatedGroup, TdxLayoutAlignmentConstraint]);
