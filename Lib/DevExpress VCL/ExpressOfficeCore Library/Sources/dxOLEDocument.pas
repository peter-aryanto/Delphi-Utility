...
const
  ...
  oleFATSector     = Integer($FFFFFFFD);        // special block begining
  ...
  oleMiniSectorCutOffSize = Integer($000001000); // max minisector size
...
function TdxOLEDocument.CreateDirEntry(const AName: Widestring; AEntryType: TdxOLEDocumentDirectoryEntryType = ET_EMPTY;
  ...
  procedure LinkValue(var AField: TdxOLEDocumentDirectoryEntry;
    const ALink, ARelaterdLink: TdxOLEDocumentDirectoryEntry);
  begin
    AField := ALink;
    if ARelaterdLink.Color = EC_RED then
...
function TdxOLEDocument.FindByName(const AName: Widestring;
  AParent: TdxOLEDocumentDirectoryEntry = nil): TdxOLEDocumentDirectoryEntry;
begin
...
begin
  ...
  FillIndexes(FDirEntryFirstSector, FDirEntryCount, False);                    // FAT for directry
