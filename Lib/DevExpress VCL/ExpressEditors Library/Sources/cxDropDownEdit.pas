...
type
  ...
  TcxCustomEditPopupWindowViewInfo = class(TdxCustomIncrementalFilteringPopupWindowViewInfo)
  strict private
  ...
  private
    ...
    procedure OnInitializaLookupDataTimer(Sender: TObject);
  protected
...
begin
  ...
  if FInitializeLookupDataTimer <> nil then
    OnInitializaLookupDataTimer(nil);
end;
...
begin
  ...
  begin
    ...
    FInitializeLookupDataTimer.OnTimer := OnInitializaLookupDataTimer;
  end;
end;
...
procedure TcxCustomDropDownEdit.OnInitializaLookupDataTimer(Sender: TObject);
begin
  FreeAndNil(FInitializeLookupDataTimer);
  InitializeLookupData;
end;
...
begin
  ...
  begin
    ...
    begin
      ASearchEditOffsets := GetSearchEditOffsets;
...
begin
  if (FIncrementalFilteringHelper.SearchEdit <> nil) and (SearchText <> '') then
    Result := FIncrementalFilteringHelper.SearchEdit.Height + cxMarginsHeight(GetSearchEditOffsets)
  else
    Result := 0;
end;

function TcxComboBoxLookupData.GetSearchEditOffsets: TRect;
begin
