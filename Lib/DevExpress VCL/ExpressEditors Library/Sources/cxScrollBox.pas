...
begin
  ...
  if AHorzScrollBarVisible then
    Dec(FClientRect.Bottom, ScrollBox.GetHScrollBarDefaultAreaHeight);
  if ScrollBox.VertScrollBar.IsVisible then
  begin
    Dec(FClientRect.Right, ScrollBox.GetVScrollBarDefaultAreaWidth);
...
begin
  ...
  begin
    ...
    try
      ACanvas.WindowOrg := cxPointInvert(AClientOffset);
      if IsScrollBarActive(sbVertical) then
