...
function GetBlobText(const Value: TcxEditValue; AProperties: TcxCustomBlobEditProperties; AFullText: Boolean): WideString;

implementation

uses
  Variants, Math, cxDrawTextUtils, cxEditUtils, cxGeometry, cxVariants, dxCore, dxThemeConsts, cxLookAndFeelPainters,
  dxDPIAwareUtils;
...
begin
  ...
  begin
    FCancelButton.SetBounds(PopupWindow.Width - ADelta,
      R.Top + (PopupWindow.ViewInfo.SysPanelHeight - AHeight) div 2 - 2, FButtonWidth, AHeight);
    FCancelButton.Anchors := [akTop, akRight];
  end
  else
  begin
    FCancelButton.SetBounds(PopupWindow.Width - ADelta,
      R.Bottom - (AHeight + (PopupWindow.ViewInfo.SysPanelHeight - AHeight) div 2 - 2),
