...
begin
  ...
  cxTextOut(Canvas.Canvas, AText, R, CXTO_LEFT or CXTO_CENTER_VERTICALLY or CXTO_SINGLELINE or CXTO_END_ELLIPSIS,
    ASelStart, Length(Edit.LookupData.SearchText), Canvas.Font,
...
procedure TcxCustomTextEditLookupData.DoInitialize(
  AVisualControlsParent: TWinControl);
begin
...
begin
  ...
  begin
    if (ActiveProperties.EditingStyle <> esFixedList) or
      (ActiveProperties.IncrementalFiltering) and (ifoUseContainsOperator in ActiveProperties.IncrementalFilteringOptions) then
