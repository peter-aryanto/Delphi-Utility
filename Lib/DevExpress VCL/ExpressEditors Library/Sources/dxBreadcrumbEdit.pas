...
begin
  ...
  begin
    FSeparatorRect := cxRectSetRight(R, R.Right, Painter.BreadcrumbEditScaledButtonAreaSeparatorSize(ScaleFactor));
...
begin
  ...
  begin
    AImageAreaSize := dxGetImageSize(Properties.Images, ScaleFactor).cx + 2 * dxBreadcrumbEditImageBorder;
    FImageRect := cxRectCenter(cxRectSetWidth(R, AImageAreaSize), dxGetImageSize(Properties.Images, ScaleFactor));
...
begin
  ...
  begin
    ...
    try
      ...
      Painter.DrawBreadcrumbEditScaledProgressChunk(ACanvas, ChunkRect, ScaleFactor);
...
begin
  if (Controller.Control.GetProperties.DropDownIndent = ddiExplorerLike) and
    (Controller.DropDownMenuOwner.Index > 0) then
  begin
    APosition.X := OwnerScreenBounds.Right -
      (Controller.DropDownMenuOwner.GetDelimiterSize + dxBreadcrumbEditDefaultDropDownIndentValue);
...
begin
  ...
  Painter.DrawBreadcrumbEditScaledDropDownButton(ACanvas, AButtonInfo.Bounds, StatesMap[AButtonInfo.Data.State], True, ScaleFactor);
  Painter.DrawBreadcrumbEditScaledDropDownButtonGlyph(ACanvas, AButtonInfo.Bounds, StatesMap[AButtonInfo.Data.State], True, ScaleFactor);
end;
