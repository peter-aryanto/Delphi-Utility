...
type
  ...
  protected
    ...
    //edit key handlers
    procedure AssignOverridableEditEvents(AEdit: TcxFindPanelMRUEdit); virtual;
...
begin
  ...
  CheckHandle;
  Visible := True;
  if FindPanel.FocusedItem = fpfiFindEdit then
    SetFocus;
end;
...
begin
  if Visible and IsFocused and not (AIsFocused or Edit.IsFocused) then
...
procedure TcxCustomFindPanel.AssignOverridableEditEvents(AEdit: TcxFindPanelMRUEdit);
begin
  AEdit.OnAfterKeyDown := EditAfterKeyDown;
  AEdit.OnKeyDown := EditKeyDown;
end;

procedure TcxCustomFindPanel.EditAfterKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_ESCAPE:
      begin
...
procedure TcxCustomFindPanel.EditKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  KeyDown(Key, Shift);
end;
...
begin
  ...
  AssignOverridableEditEvents(Result);
