...
begin
  R := cxRectContent(R, ScaleFactor.Apply(GetTextRectCorrection));
end;
...
procedure TcxCustomRadioGroupViewInfo.DrawButtonCaption(ACanvas: TcxCanvas;
  ...
  begin
    if AButtonViewInfo.Data.NativeStyle then
      R := cxRectContent(R, ScaleFactor.Apply(ANativeStyleTextRectCorrection))
    else
      R := cxRectContent(R, ScaleFactor.Apply(ATextRectCorrection));
