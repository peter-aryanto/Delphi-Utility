...
{ TcxMonthListBox - Bug in Delphi5: Must be in the interface part }
  ...
  protected
    ...
    property HorzAlignment: TAlignment read FHorzAlignment;
  ...
  private
    ...
    procedure CalculateMaxYearAndMonthNameLengths(var AMaxYearNameWidth: Integer;
      var AMaxMonthNameWidth: Integer);
  protected
    procedure AddVisibleElements; override;
    procedure CreateElements; override;
    procedure DestroyElements; override;
  ...
  protected
    ...
    procedure DrawBackround(ACanvas: TcxCanvas); virtual;
  ...
  protected
    ...
    function IsHeaderVisible: Boolean; override;
    procedure SetFirstDate(Value: Double; AAnimated: Boolean); override;
  public
    procedure Paint(ACanvas: TcxCanvas); override;
  end;
...
begin
  ...
  begin
    ...
    ATextRect.Right := ATextRect.Left + DateCells[0].Size.cx;
...
begin
  AScale := dxSizeF(
    StartState.Scale.cx  + FScaleKoef.cx * APosition,
    StartState.Scale.cy  + FScaleKoef.cy * APosition);
  AOffset := dxPointF(
    StartState.Offset.X + FOffsetKoef.X * APosition,
    StartState.Offset.Y + FOffsetKoef.Y * APosition);
  ...
  FCurrentAlphaValue := Trunc(StartState.Alpha + FAlphaKoef * APosition);
end;
...
begin
  ...
  begin
    ...
    begin
      if Value < Calendar.FirstDate then
        FDirection := dirRight
      else
        FDirection := dirLeft;
...
begin
  ...
  begin
    ...
    begin
      if AValue < Calendar.SelectDate then
        FDirection := dirRight
      else
        FDirection := dirLeft;
      RecreateImage(FStartImage);
...
begin
  ...
  SynchronizeFirstDate;
  Recalculate;
  FNexSelectCellBounds := GetCurrentTableViewInfo.GetSelectedCell.Bounds;
end;
...
begin
  ...
  begin
    AState.Scale := dxSizeF(cxRectWidth(FNexSelectCellBounds)/cxRectWidth(FTableAreaBounds),
      cxRectHeight(FNexSelectCellBounds)/cxRectHeight(FTableAreaBounds));
    AState.Offset := dxPointF(GetRectCenterOffset(FTableAreaBounds, FNexSelectCellBounds));
  end
  else
  begin
  ...
  if FDirection = dirUp then
  begin
    AState.Scale := dxSizeF(cxRectWidth(FTableAreaBounds)/cxRectWidth(FNexSelectCellBounds),
      cxRectHeight(FTableAreaBounds)/cxRectHeight(FNexSelectCellBounds));
    AOffset := GetRectCenterOffset(FNexSelectCellBounds, FTableAreaBounds);
...
procedure TcxCalendarViewInfo.DrawBackround(ACanvas: TcxCanvas);
begin
...
var
  ASkinPainter: TcxCustomLookAndFeelPainter;
begin
  ASkinPainter := FCalendar.LookAndFeel.SkinPainter;
  Result := (ASkinPainter <> nil) or (LookAndFeelPainter.LookAndFeelStyle = lfsNative);
end;
...
procedure TcxCalendarTouchUIViewInfo.SetFirstDate(Value: Double; AAnimated: Boolean);
begin
// do nothing
end;
...
begin
  ...
  LookAndFeelPainter.CalculateArrowPoints(R, P, AViewInfo.Direction, False);
...
procedure TcxCalendarPainter.DrawDateHeaderCell(ACanvas: TcxCanvas;
  ...
  var
    ...
    ATextRect: TRect;
  begin
    ACanvas.Font.Color := LookAndFeelPainter.DefaultDateNavigatorHeaderTextColor(AIsHighlighted);
    FCalendar.CorrectHeaderTextRect(R);
    ATextSize := ACanvas.TextExtent(S);
    ATextRect := cxRectCenter(R, ATextSize);
    ACanvas.Canvas.TextRect(R, ATextRect.Left, ATextRect.Top, S);
  end;
...
begin
  ...
  DrawHeaderText(AViewInfo.GetText, AViewInfo.Bounds, AViewInfo.State = cesHot);
  ACanvas.Brush.Style := bsSolid;
end;
...
procedure TcxCalendarPainter.DrawHeader(ACanvas: TcxCanvas;
...
begin
  LookAndFeelPainter.DrawScaledModernCalendarArrow(ACanvas,
    AViewInfo.Bounds, AViewInfo.Direction, AViewInfo.State, FCalendar.ScaleFactor);
end;
