...
begin
  ...
  AToggleSwitchViewInfo.OffIndicatorRect := CalculateOffIndicatorRect(AToggleSwitchViewInfo);
  AToggleSwitchViewInfo.OnIndicatorRect := CalculateOnIndicatorRect(AToggleSwitchViewInfo);
end;
...
begin
  ...
  begin
    case AToggleSwitchViewInfo.Alignment of
      taLeftJustify:
        if AToggleSwitchViewInfo.OffIndicatorRectSize.cx > 0 then
          Result := cxRectSetLeft(Result,
            Result.Left + AToggleSwitchViewInfo.OffIndicatorRectSize.cx + GetStateIndicatorGap(AToggleSwitchViewInfo));
      taRightJustify:
        if AToggleSwitchViewInfo.OnIndicatorRectSize.cx > 0 then
          Result := cxRectSetRight(Result,
            Result.Right - AToggleSwitchViewInfo.OnIndicatorRectSize.cx - GetStateIndicatorGap(AToggleSwitchViewInfo));
    end;
  end;
end;
  ...
  if (cxRectWidth(Result) > 0) and (AToggleSwitchViewInfo.StateIndicatorKind <> sikNone) and
    (AToggleSwitchViewInfo.StateIndicatorPosition = sipOutside) then
      case AViewInfo.Alignment of
        taLeftJustify:
          if AToggleSwitchViewInfo.OnIndicatorRectSize.cx > 0 then
            Result.Left := Result.Left + AToggleSwitchViewInfo.OnIndicatorRectSize.cx + GetStateIndicatorGap(AToggleSwitchViewInfo);
        taRightJustify:
          if AToggleSwitchViewInfo.OffIndicatorRectSize.cx > 0 then
            Result.Right := Result.Right - AToggleSwitchViewInfo.OffIndicatorRectSize.cx - GetStateIndicatorGap(AToggleSwitchViewInfo);
      end;
end;
...
function TdxCustomToggleSwitchViewData.CalculateFocusRect(AViewInfo: TdxCustomToggleSwitchViewInfo): TRect;
  ...
  procedure CheckFocusRectBounds(var AFocusRect: TRect);
  var
    AMaxRect: TRect;
  begin
    with AViewInfo do
    begin
      if Alignment = taCenter then
        AMaxRect := Rect(AFocusRect.Left, ClientRect.Top, AFocusRect.Right, ClientRect.Bottom)
      else
      begin
        AMaxRect := Rect(TextRect.Left - 1, TextRect.Top - 1, TextRect.Right + 1, TextRect.Bottom + 1);
        AMaxRect.Right := Min(AMaxRect.Right, BorderRect.Right - 1);
      end;

      AFocusRect.Left := Max(AFocusRect.Left, AMaxRect.Left);
      AFocusRect.Top := Max(AFocusRect.Top, AMaxRect.Top);
      AFocusRect.Right := Min(AFocusRect.Right, AMaxRect.Right);
      AFocusRect.Bottom := Min(AFocusRect.Bottom, AMaxRect.Bottom);
    end;
  end;
...
begin
  ...
  begin
    case AViewInfo.StateIndicatorPosition of
      sipInside:
        begin
          Result := cxRectCenterVertically(AViewInfo.CheckBoxRect, AViewInfo.OffIndicatorRectSize.cy);
          Result := cxRectSetWidth(Result, AViewInfo.OffIndicatorRectSize.cx);
          Result := cxRectSetRight(Result, AViewInfo.CheckBoxRect.Right - GetStateIndicatorGap(AViewInfo));
        end;
      sipOutside:
        begin
          Result := cxRectCenterVertically(AViewInfo.ClientRect, AViewInfo.OffIndicatorRectSize.cy);
          Result := cxRectSetWidth(Result, AViewInfo.OffIndicatorRectSize.cx);
...
begin
  ...
  begin
    case AViewInfo.StateIndicatorPosition of
      sipInside:
        begin
          Result := cxRectCenterVertically(AViewInfo.CheckBoxRect, AViewInfo.OnIndicatorRectSize.cy);
          Result := cxRectSetWidth(Result, AViewInfo.OnIndicatorRectSize.cx);
          Result := cxRectSetLeft(Result, AViewInfo.CheckBoxRect.Left + GetStateIndicatorGap(AViewInfo));
        end;
      sipOutside:
        begin
          Result := cxRectCenterVertically(AViewInfo.ClientRect, AViewInfo.OnIndicatorRectSize.cy);
          Result := cxRectSetWidth(Result, AViewInfo.OnIndicatorRectSize.cx);
