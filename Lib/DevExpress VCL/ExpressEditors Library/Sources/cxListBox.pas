...
type
  ...
  protected
    ...
    procedure DrawItemCheck(const R: TRect; AItem: TdxCustomListBoxItem; AState: TcxButtonState); virtual;
    procedure DrawItemFocus(const R: TRect; AItem: TdxCustomListBoxItem); virtual;
    function GetAutoSizeWidth(const AUseCachedTextMaxWidth: Boolean = False): Integer;
    function GetCheckedCount: Integer;
    function GetCheckedItems: TList;
    function GetDragItemInsertionIndex(X, Y: Integer): Integer;
    function GetEditValueAsString: string;
    function GetImagesAreaRect: TRect; override;
    function GetIsCopyDragDrop: Boolean; override;
    function GetItemCheckRect(const AItemRect: TRect): TRect;
    ...
    function HitAtItemCheckArea(AItemIndex: Integer): Boolean;
  ...
  TdxQuickCustomizationCheckListBoxAction = procedure(Sender: TdxQuickCustomizationCustomCheckListBox;
  ...
  private
    ...
    FOnAction: TdxQuickCustomizationCheckListBoxAction;
    FOnSelectedItemCheckedStateChanged: TNotifyEvent;
    FOnSelectedItemCheckedStateChanging: TNotifyEvent;
    procedure WMLButtonDblClk(var Message: TWMLButtonDblClk); message WM_LBUTTONDBLCLK;
  protected
    procedure ApplyClick(AItemIndex: Integer);
    procedure DblClick; override;
    procedure DoClick(Sender: TObject); override;
    procedure InternalMouseDown(Button: TMouseButton; Shift: TShiftState; X, Y: Integer); override;
    function NeedHandleClick: Boolean; override;
    function ProcessNavigationKey(var Key: Word; Shift: TShiftState): Boolean; override;
    procedure SetSelectedCheckedState(AValue: Boolean);
  public
    constructor Create(AOwner: TComponent); override;

    property OnAction: TdxQuickCustomizationCheckListBoxAction read FOnAction write FOnAction;
...
begin
  ...
  begin
    R := GetItemRect(I, R);
    if R.Bottom > ItemsAreaRect.Bottom then
      Break;
...
procedure TdxCustomCheckListBox.DrawItem(const R: TRect; AItem: TdxCustomListBoxItem; AState: TcxButtonState);
  ...
  function GetItemBackgroundRect(const R: TRect): TRect;
  begin
    Result := R;
    Inc(Result.Left, GetCheckAreaWidth);
  end;
...
begin
  GetItemPartsRects(AItem, R, AItemRect, ASeparatorRect);
  DrawItemBackground(GetItemBackgroundRect(AItemRect), AItem, AState);
  if MultiSelect and (AItem.Index = ItemIndex) and not Selected[ItemIndex] then
    DrawItemFocus(AItemRect, AItem);
  DrawItemImage(GetItemImageRect(AItemRect), AItem, AState);
  if HasCheckBox(AItem.Index) then
    DrawItemCheck(GetItemCheckRect(AItemRect), AItem, AState);
  DrawItemText(GetItemTextRect(AItemRect), AItem, AState);
  DrawItemSeparator(ASeparatorRect);
end;

procedure TdxCustomCheckListBox.DrawItemCheck(const R: TRect; AItem: TdxCustomListBoxItem; AState: TcxButtonState);
var
  ARect: TRect;
begin
  ARect := cxRectCenter(R, LookAndFeelPainter.ScaledCheckButtonSize(ScaleFactor));
  LookAndFeelPainter.DrawScaledCheckButton(Canvas, ARect, AState, (AItem as TdxCustomCheckListBoxItem).State, ScaleFactor);
end;

procedure TdxCustomCheckListBox.DrawItemFocus(const R: TRect; AItem: TdxCustomListBoxItem);
var
  ARect: TRect;
begin
  ARect := R;
  if HasCheckBox(AItem.Index) then
    ARect.Left := GetItemCheckRect(R).Right;
  Canvas.DrawFocusRect(cxRectInflate(ARect, -1, -1));
end;

function TdxCustomCheckListBox.GetAutoSizeWidth(const AUseCachedTextMaxWidth: Boolean = False): Integer;
var
...
begin
  ...
  if AUseCachedTextMaxWidth then
    AItemWidth := FCachedTextMaxWidth
  else
  begin
    AItemWidth := 0;
    cxScreenCanvas.Font := Font;
    for I := 0 to Count - 1 do
      AItemWidth := Max(AItemWidth, cxTextWidth(cxScreenCanvas.Font, Items[I].Caption));
    cxScreenCanvas.Dormant;
    FCachedTextMaxWidth := AItemWidth;
  end;
  ATextOffsets := TextOffsets;
  Inc(Result, AItemWidth + ATextOffsets.Left + ATextOffsets.Right);
end;

function TdxCustomCheckListBox.GetCheckAreaWidth: Integer;
begin
  Result := 2 * ScaleFactor.Apply(2) + LookAndFeelPainter.ScaledCheckButtonSize(ScaleFactor).cx;
...
function TdxCustomCheckListBox.GetEditValueAsString: string;
var
  I: Integer;
begin
  SetLength(Result, Count);
  for I := 0 to Count - 1 do
    Result[I + 1] := Char(Integer(Items[I].State) + Ord('0'));
end;

function TdxCustomCheckListBox.GetImagesAreaRect: TRect;
begin
  Result := cxRectOffsetHorz(inherited GetImagesAreaRect, GetCheckAreaWidth);
end;
...
function TdxCustomCheckListBox.GetItemCheckRect(const AItemRect: TRect): TRect;
begin
  Result := AItemRect;
  Result.Right := Result.Left + GetCheckAreaWidth;
end;
...
function TdxCustomCheckListBox.HitAtItemCheckArea(AItemIndex: Integer): Boolean;
begin
  Result := Items.IsValidIndex(AItemIndex) and HasCheckBox(AItemIndex) and
   PtInRect(GetItemCheckRect(ItemRect(AItemIndex)), ScreenToClient(GetMouseCursorPos));
end;
...
begin
  ...
  AHeight := LookAndFeelPainter.ScaledCheckButtonSize(ScaleFactor).cy;
  dxAdjustToTouchableSize(AHeight, ScaleFactor);
...
begin
  ...
  if HitAtItemCheckArea(ItemIndex) then
    SelectFocusedItem
  else
...
begin
  Result := Selected[AItem.Index] or ((AState = cxbsPressed) and not MultiSelect);
end;
...
begin
  ...
  with ScreenToClient(GetMouseCursorPos) do
    if Assigned(OnDragOver) then
      OnDragOver(nil, nil, X, Y, dsDragMove, AAccept);
end;
...
begin
  ...
  LookAndFeelPainter.DrawDropDownListBoxScaledGutterBackground(Canvas, ImagesAreaRect, ScaleFactor);
end;
...
begin
  if AState = cxbsPressed then
    LookAndFeelPainter.DrawDropDownListBoxScaledSelection(Canvas, R, ImagesAreaRect, ScaleFactor);
end;
...
procedure TdxQuickCustomizationCustomCheckListBox.ClearClickedState;
var
...
procedure TdxQuickCustomizationCustomCheckListBox.Click;
begin
...
procedure TdxQuickCustomizationCustomCheckListBox.DoClick(Sender: TObject);
begin
//
end;
...
begin
  ...
  if APrevIndex <> AIndex then
    GetQuickCustomizationControl.Controller.HandleItemIndexChanging(Self, AIndex);
end;

{ TdxQuickCustomizationCommandListBox }

procedure TdxQuickCustomizationCommandListBox.DblClick;
begin
  if HasCheckBox(ItemIndex) then
    DoClick(Self);
end;

procedure TdxQuickCustomizationCommandListBox.DoClick(Sender: TObject);
var
  ACommand: TdxQuickCustomizationCustomCommand;
begin
  if Items.IsValidIndex(ItemIndex) then
  begin
    ACommand := Items[ItemIndex].Data as TdxQuickCustomizationCustomCommand;
    if ACommand.HasCheckBox then
    begin
      Checked[ItemIndex] := not Checked[ItemIndex];
      ACommand.Checked := Checked[ItemIndex];
    end;
    ACommand.Execute;
  end;
end;
...
begin
  Result := Items.IsValidIndex(AItemIndex) and
    (Items[AItemIndex].Data as TdxQuickCustomizationCustomCommand).HasCheckBox;
end;

function TdxQuickCustomizationCommandListBox.NeedHandleClick: Boolean;
begin
  Result := (csClicked in ControlState) and (not HasCheckBox(ItemIndex) or HitAtItemCheckArea(ItemIndex));
end;

function TdxQuickCustomizationCommandListBox.ProcessNavigationKey(var Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
  case Key of
    VK_RETURN, VK_SPACE:
...
procedure TdxQuickCustomizationCheckListBox.DblClick;
begin
  ApplyClick(ItemIndex);
end;

procedure TdxQuickCustomizationCheckListBox.DoClick;
begin
  ApplyClick(ItemIndex);
end;
...
function TdxQuickCustomizationCheckListBox.NeedHandleClick: Boolean;
begin
  Result := (csClicked in ControlState) and HitAtItemCheckArea(ItemIndex);
end;

function TdxQuickCustomizationCheckListBox.ProcessNavigationKey(var Key: Word; Shift: TShiftState): Boolean;
begin
  Result := True;
  case Key of
    VK_SPACE:
      if (GetSelectionCount >= 1) and Selected[ItemIndex] then
        SetSelectedCheckedState(not Checked[ItemIndex])
      else
        ApplyClick(ItemIndex);
    else
      Result := GetQuickCustomizationControl.Controller.ProcessNavigationKey(Self, Key, Shift) or
...
procedure TdxQuickCustomizationCheckListBox.SetSelectedCheckedState(AValue: Boolean);
var
  I, AIndex: Integer;
begin
...
var
  ...
  ATop2: Integer;
begin
  AHeight1 := CommandListBox.ItemHeight * CommandListBox.Items.Count;
  AWidth1 := CommandListBox.GetAutoSizeWidth(AUseCachedTextMaxWidth);
  ATop2 := AHeight1;
  if CommandListBox.Items.Count > 0 then
    Inc(ATop2, GetSeparatorHeight);

  if (CheckListBoxVisibleRowCount <= 0) or (CheckListBox.Items.Count < CheckListBoxVisibleRowCount) then
    FCheckListBoxVisibleRowCount := CheckListBox.Items.Count;

  AHeight2 := CheckListBoxVisibleRowCount * CheckListBox.ItemHeight;
  AWidth2 := CheckListBox.GetAutoSizeWidth(AUseCachedTextMaxWidth);
  if CheckListBoxVisibleRowCount < CheckListBox.Items.Count then
    Inc(AWidth2, CheckListBox.VScrollBar.Width);
