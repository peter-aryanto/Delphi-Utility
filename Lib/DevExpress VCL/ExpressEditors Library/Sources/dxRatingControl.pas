...
type
  ...
  public
    ...
    function GetDisplayText(const AEditValue: Variant; AFullText: Boolean = False; AIsInplace: Boolean = True): WideString; override;
...
begin
  ...
  try
    ...
    DrawEditBackground(ABufferBitmap.cxCanvas, Bounds, cxNullRect, False);
    DrawText(ABufferBitmap.cxCanvas);
    ABufferBitmap.Canvas.DrawFocusRect(FocusRect);
    DrawCustomEditValidationMark(ABufferBitmap.cxCanvas);
    DrawIndicators(ABufferBitmap.cxCanvas);
    ABufferBitmap.cxCanvas.WindowOrg := cxNullPoint;
    cxBitBlt(ACanvas.Handle, ABufferBitmap.cxCanvas.Handle, Bounds, cxNullPoint, SRCCOPY);
  finally
    ABufferBitmap.Free;
  end;
end;
...
function TdxCustomRatingControl.GetDisplayValue: string;
begin
