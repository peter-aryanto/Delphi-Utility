...
type
  ...
  protected
    ...
    procedure CheckCursor(const AMousePos: TPoint); override;
...
procedure TcxHyperLinkEditViewInfo.CheckCursor(const AMousePos: TPoint);
begin
...
begin
  if not DoOnStartClick then
    OpenHyperLink(DisplayValue, ActiveProperties);
end;
...
begin
  if IsDesigning then
    ...
    if TcxHyperLinkEditViewInfo(ViewInfo).CanHandPointCursor(Point(X, Y)) then
