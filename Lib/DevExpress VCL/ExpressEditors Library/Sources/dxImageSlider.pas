...
begin
  Result := Options.Position in [posRight, posLeft];
end;
...
begin
  ...
  begin
    ...
    if Options.Position in [posLeft, posRight] then
      Options.Size := Options.ActuallySize + (R.Left - TdxCustomImageSlider(Control).GetPreviewSplitterBounds.Left) * Sign[Options.Position = posLeft]
    else
      Options.Size := Options.ActuallySize + (R.Top - TdxCustomImageSlider(Control).GetPreviewSplitterBounds.Top) * Sign[Options.Position = posTop];
  end;
...
begin
   ...
   ACenter := cxRectCenter(TdxCustomImageSlider(Control).BoundsRect);
   case Options.Position of
     ...
     posRight:
     begin
       Result.Right := ImageSlider.BoundsRect.Right - Options.MinWidth;
     ...
     posBottom:
     begin
       Result.Bottom := ImageSlider.BoundsRect.Bottom - Options.MinHeight;
...
begin
  ...
  LookAndFeelPainter.DrawScaledSliderButton(Canvas, AViewInfo.Bounds, ADirection[AViewInfo.ObjectID], AState, ScaleFactor)
end;
...
begin
  ...
  LookAndFeelPainter.DrawScaledSplitter(ACanvas, AViewInfo.Bounds, AViewInfo = HotTrackItem,
    (AViewInfo = HotTrackItem) and MousePressed, PreviewOptions.Position in [posTop, posBottom], ScaleFactor);
...
begin
  if FPreviewScrollBar = nil then
    Exit;
  PreviewScrollBar.Kind := AKind[PreviewOptions.Position in [posLeft, posRight]];
...
begin
  Result := (PreviewOptions.Position <> posNone) and (Count >= 1);
end;
...
const
  AContentPosition: array[TcxPosition] of TcxPosition = (posNone, posLeft, posLeft, posTop, posTop);
...
begin
  ...
  APreviewBounds := GetPartBounds(GetPreviewBounds, AContentPosition[PreviewOptions.Position],
    GetPreviewSize - GetPreviewSplitterSize - IfThen(not IsPopupScrollBars, GetPreviewScrollBarSize));
  if PreviewOptions.Position = posRight then
    OffsetRect(APreviewBounds, GetPreviewSplitterSize, 0)
  else
    if PreviewOptions.Position = posBottom then
...
begin
  if PreviewOptions.Position = posNone then
    Result := inherited GetClientBounds
  else
  begin
    Result := cxRectInflate(Bounds, -BorderSize);
    case PreviewOptions.Position of
      posLeft:
    ...
    begin
      if HScrollBarVisible then
        Result.Bottom := GetHScrollBarBounds.Top;
      if VScrollBarVisible then
        Result.Right := GetVScrollBarBounds.Left;
    end;
  end;
end;
...
begin
  ...
  if CheckSplitterHitTest(X, Y) then
    Result := SplitterCursors[PreviewOptions.Position in [posRight, posLeft]]
end;
...
function TdxCustomImageSlider.GetHScrollBarBounds: TRect;
begin
  Result := cxRectInflate(Bounds, -BorderSize);
  ...
  case PreviewOptions.Position of
    ...
    posRight:
      Result.Right := GetPreviewBounds.Left;
    posTop:
      Result := cxRectSetTop(Result, Max(Result.Top, GetPreviewBounds.Bottom));
    posBottom:
      Result := cxRectSetBottom(Result, GetPreviewBounds.Top);
  end;
end;
...
begin
  Result := AWheelScrollKind[PreviewOptions.Position];
end;
...
begin
  ...
  Result := GetPartBounds(cxRectInflate(Bounds, -BorderSize), PreviewOptions.Position, GetPreviewSize);
end;
...
begin
  Result := GetPartBounds(GetPreviewBounds, AScrollBarPosition[PreviewOptions.Position], GetPreviewScrollBarSize);
  if PreviewOptions.Position = posLeft then
    OffsetRect(Result, -GetPreviewSplitterSize, 0)
  else
    if PreviewOptions.Position = posTop then
      OffsetRect(Result, 0, -GetPreviewSplitterSize);
end;
...
begin
  Result := GetPartBounds(GetPreviewBounds, ASplitterPosition[PreviewOptions.Position], GetPreviewSplitterSize);
end;

function TdxCustomImageSlider.GetVScrollBarBounds: TRect;
begin
  Result := cxRectInflate(Bounds, -BorderSize);
  Result.Left := Result.Right - GetScrollBarSize.cx;
  case PreviewOptions.Position of
    posLeft:
      Result := cxRectSetLeft(Result, Max(Result.Left, GetPreviewBounds.Right));
    posRight:
      Result := cxRectSetRight(Result, GetPreviewBounds.Left);
    posTop:
      Result.Top := GetPreviewBounds.Bottom;
    posBottom:
      Result.Bottom := GetPreviewBounds.Top;
...
{   todo: // for top bottom buttons
    ...
    begin
      FButtonPrev := AddViewInfo(cxRectBounds(R.Left + AOffset.cx, (R.Top + R.Bottom - AButtonSize.cy) div 2,
        AButtonSize.cx, AButtonSize.cy), 0, DrawButton, NavigationButtonClick);
      FButtonNext := AddViewInfo(cxRectBounds(R.Right - AButtonSize.cx - AOffset.cx, (R.Top + R.Bottom - AButtonSize.cy) div 2,
        AButtonSize.cx, AButtonSize.cy), 1, DrawButton, NavigationButtonClick);
    end;
  end;
...
begin
  ...
  if PreviewOptions.Position in [posLeft, posRight] then
...
var
  ASize: TSize;
begin
  if (PreviewOptions.Position = posNone) or not PreviewOptions.Resizable then
    Result := 0
  else
  begin
    ASize := LookAndFeelPainter.GetScaledSplitterSize(PreviewOptions.Position in [posTop, posBottom], ScaleFactor);
    if PreviewOptions.Position in [posTop, posBottom] then
...
begin
  Result := False;
  if FSlider.PreviewOptions.Position in [posLeft, posRight] then
    Result := AScrollKind = sbVertical
  else
    if FSlider.PreviewOptions.Position in [posTop, posBottom] then
      Result := AScrollKind = sbHorizontal;
end;
