...
procedure TcxMCListBox.DrawItem(Control: TWinControl; Index: Integer; ARect: TRect; State: TOwnerDrawState);
  ...
  begin
    InnerListBox.DrawLine(GetHeaderSectionRect(AColumnIndex, InnerListBox.VScrollBarVisible).Right, ARect.Top, ARect.Bottom);
  end;
...
procedure TcxMCListBox.AdjustChildsPosition;
  ...
  begin
    Result := FInnerPanel.ClientWidth;
    if InnerListBox.VScrollBarVisible then
      Dec(Result, FInnerListBoxContainer.GetVScrollBarDefaultAreaWidth);
  end;

begin
  InnerHeader.SetBounds(
    0, 0, GetInnerHeaderWidth, InnerHeader.Height);
