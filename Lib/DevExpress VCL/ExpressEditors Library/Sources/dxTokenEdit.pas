...
uses
  ...
  dxCore, dxCoreClasses, cxClasses, cxControls, cxGraphics, cxGeometry, cxLookAndFeelPainters, cxContainer, cxEdit,
  cxTextEdit, cxGroupBox, dxAutoCompleteWindow, dxFading, dxSmartImage, dxGDIPlusClasses, cxListBox, cxDataUtils;
...
uses
  RTLConsts, cxEditConsts, StrUtils, dxDPIAwareUtils;
...
begin
  ...
  try
    ...
    AEditValueBuilder := TStringBuilder.Create;
    try
...
begin
  FBounds := ABounds;
  FCaptionBounds := cxRectContent(ABounds, Painter.GetScaledTokenContentOffsets(ScaleFactor));
end;
  ...
  begin
    ...
    try
      ...
      begin
        ...
        ACanvas.IntersectClipRect(cxRectContent(Bounds, Painter.GetScaledTokenContentOffsets(ScaleFactor)));
        DrawContent(ACanvas);
      end;
    finally
...
var
  AMargins: TRect;
begin
  AMargins := Painter.GetScaledTokenContentOffsets(ScaleFactor);
...
begin
  ...
  begin
    ...
    ACanvas.Font.Color := GetTextColor(State);
    ACanvas.DrawTexT(Caption, CaptionBounds, CaptionAlignment, vaCenter, False, True);
  end;
end;
...
begin
  ...
  begin
    ...
    Result.cy := Max(Result.cy, ASize.cy + cxMarginsHeight(Painter.GetScaledTokenContentOffsets(ScaleFactor)));
  end;
  if IsGlyphVisible then
  begin
    ASize.cy := Result.cy - cxMarginsHeight(Painter.GetScaledTokenContentOffsets(ScaleFactor));
    ASize := cxSizeProportionalStretch(cxSize(ASize.cy, ASize.cy), GlyphSize);
    Result.cx := Result.cx + ASize.cx + ScaleFactor.Apply(GlyphOffset);
  end;
end;
...
var
  ...
  I: Integer;
begin
  AHiddenTokens := TStringBuilder.Create;
  try
    AHiddenTokenCount := 0;
...
procedure TdxTokenEditViewInfo.CalculateMoreTokensItem(const AContentRect: TRect; ARowHeight, AMinItemSize: Integer);
  ...
  function GetFreeSpaceForMoreTokensItem(AToken: TdxTokenEditTokenViewInfo; ARequiredSpace: Integer): Boolean;
    ...
    begin
      Result := ARequiredSpace - (AContentRect.Right - (AToken.Bounds.Right + IndentBetweenItems));
    end;
  ...
  begin
    ...
    begin
      ...
      ATokenBounds := cxRectSetWidth(ATokenBounds, Max(0, cxRectWidth(ATokenBounds) - GetNeededSpace));
      if IsTooSmallForDisplay(ATokenBounds, AContentRect, AToken.MeasureSize.cx, AMinItemSize) then
        ATokenBounds := cxRectSetWidth(ATokenBounds, 0);
...
begin
  ...
  begin
    ...
    if AToken <> nil then
      AItemBounds := cxRectBounds(AToken.Bounds.Right + IfThen(AToken.IsVisible, IndentBetweenItems), AToken.Bounds.Top, 0, 0)
    else
      AItemBounds := AContentRect;

    MoreTokensItem.Calculate(cxRectSetSize(AItemBounds, AItemSize.cx, ARowHeight));
  end
  else
...
function TdxCustomTokenEdit.GetDisplayValue: string;
begin
