...
procedure CalculateCustomCheckBoxViewInfo(AViewData: TcxCustomCheckBoxViewData;
  ...
  begin
    ...
    begin
      ...
      FocusRect.Left := Max(FocusRect.Left, AMaxRect.Left);
      FocusRect.Top := Max(FocusRect.Top, AMaxRect.Top);
      FocusRect.Right := Min(FocusRect.Right, AMaxRect.Right);
      FocusRect.Bottom := Min(FocusRect.Bottom, AMaxRect.Bottom);
    end;
  end;
...
procedure DrawScaledEditCheck(ACanvas: TcxCanvas; const ACheckRect: TRect;
  ...
  begin
    ...
    try
      AGlyphWidth := AGlyph.Width div AGlyphCount;
      if Supports(AGlyph, IdxVectorImage) then
      begin
        ABitmap.Height := AScaleFactor.Apply(AGlyph.Height);
        ABitmap.Width := AScaleFactor.Apply(AGlyphWidth);
      end
      else
      begin
        ABitmap.Height := AGlyph.Height;
        ABitmap.Width := AGlyphWidth;
      end;
      ABitmap.Transparent := True;
      ADest := cxRectBounds(0, 0, ABitmap.Width, ABitmap.Height);
      ASource := cxRectBounds(AGlyphWidth * AGlyphIndex, 0, AGlyphWidth, AGlyph.Height);
      AGlyph.StretchDraw(ABitmap.Canvas.Handle, ADest, ASource);
      if ADrawBackground then
        ACanvas.FillRect(ACheckRect, ABackgroundColor);
      if Supports(AGlyph, IdxVectorImage) then
        cxDrawImage(ACanvas, ACheckRect, ABitmap, nil, -1, not (ACheckState = ecsDisabled))
      else
        cxDrawImage(ACanvas, ACheckRect, ABitmap, nil, -1, not (ACheckState = ecsDisabled), nil, AScaleFactor);
    finally
      ABitmap.Free;
    end;
  end;
