...
procedure InitRichRenderer(AProperties: TcxCustomRichEditProperties;
  AFont: TFont; AColor, ATextColor: TColor; const AText: string; AScaleFactor: TdxScaleFactor);
begin
...
begin
  ...
  try
    ...
    if AScaleFactor <> nil then
      ...
      ANotScaledFont.Height := dxSystemScaleFactor.Apply(ANotScaledFont.Height);
    ...
    dxDrawRichEdit(ADC, cxRectSetNullOrigin(ARect), RichRenderer.Handle, AScaleFactor, 0, -1, ACalculateHeight, AHeight);
  finally
    ANotScaledFont.Free;
  end;
end;
...
begin
  if not HandleAllocated then
    Exit;
  AZoomFactor := FZoomFactor * ScaleFactor.TargetDPI / dxSystemScaleFactor.TargetDPI;
...
begin
  ...
  begin
    if not IsInplace and not AreFontsEqual(InnerRich.Font, VisibleFont) then
...
begin
  inherited;
  if not IsScaleChanging then
    InnerRich.DefAttributes.Assign(VisibleFont);
end;
