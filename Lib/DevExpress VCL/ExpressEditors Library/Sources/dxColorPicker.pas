...
begin
  ...
  begin
    ...
    try
      ...
      ACanvas.DrawTexT(Caption, CaptionRect, taRightJustify, vaCenter, False, False);
    finally
...
begin
  ...
  Dec(R.Right, ScaleFactor.Apply(dxcpIndentBetweenElements));

  if FSlider2 <> nil then
  begin
    R1 := cxRectInflate(R, 0, ScaleFactor.Apply(dxcpSliderArrowWidth) - Painter.BorderSize);
    FSlider2.Bounds := cxRectSetRight(R1, R1.Right, FSlider2.MeasureWidth);
    R.Right := FSlider2.Bounds.Left - ScaleFactor.Apply(dxcpIndentBetweenElements);
  end;

  if FSlider1 <> nil then
  begin
    R1 := cxRectInflate(R, 0, ScaleFactor.Apply(dxcpSliderArrowWidth) - Painter.BorderSize);
    FSlider1.Bounds := cxRectSetRight(R1, R1.Right, FSlider1.MeasureWidth);
    R.Right := FSlider1.Bounds.Left - 2 * ScaleFactor.Apply(dxcpIndentBetweenElements);
  end;
