...
type
  ...
  strict private
    ...
    class function CheckKnownSchemes(ALptr: PChar; ALength: Integer; out AType: TSchemeType): Boolean; static;
...
begin
  Result :=
     ((AUnicode >= #$A0) and (AUnicode <= #$D7FF)) or
...
begin
  ...
  begin
    ...
    AEncodedBytesLength := TEncoding.UTF8.GetBytes(AUnescapedChars, J, 1 + Ord(AIsHighSurr), AEncodedBytes, 0);
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            else if ((ACh <= #$9F) and IsNotSafeForUnescape(ACh)) or ((ACh > #$9F) and not CheckIriUnicodeRange(ACh, AIsQuery)) then
            begin
              Inc(ANext, 3);
              Continue;
            end;
...
class function TdxURI.CheckKnownSchemes(ALptr: PChar; ALength: Integer; out AType: TSchemeType): Boolean;
var
  S: string;
  I: TSchemeType;
begin
  S := LowerCase(Copy(ALptr, 1, ALength));
...
begin
  Result := FSchemeType = TSchemeType.&FILE;
end;
