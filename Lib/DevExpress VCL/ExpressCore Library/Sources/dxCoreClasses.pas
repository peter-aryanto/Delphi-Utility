...
type
  ...
  strict private
    FHasValue: {$IFDEF DELPHIXE2} WordBool; {$ELSE} LongBool; {$ENDIF}
    FValue: T;
...
begin
  ...
  SetLength(FParamOffsets, ATypeData^.ParamCount);
  ACurReg := paEDX;
  P := @ATypeData^.ParamList;
  AOffset := FStackSize;
  for I := 0 to ATypeData^.ParamCount - 1 do
  begin
    ...
    if (Size <= 4) and (ACurReg <= paECX) then
    begin
      FParamOffsets[I] := ACurReg;
      Inc(ACurReg);
    end
    else
    begin
      Dec(AOffset, Size);
      FParamOffsets[I] := AOffset;
    end;
    NextParameter(P);
  end;
