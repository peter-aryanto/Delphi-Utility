...
type
  ...
  TdxRectFHelper = record helper for TdxRectF
  public
  ...
  public
    class function Null: TdxSizeF; static;
...
class function TdxRectFHelper.CreateSize(const ALeft, ATop, AWidth, AHeight: Single): TdxRectF;
begin
  Result.Left := ALeft;
  Result.Top := ATop;
  Result.Right := ALeft + AWidth;
...
procedure TdxRectFHelper.Init(const ALeft, ATop, ARight, ABottom: Single);
begin
  Left := ALeft;
  Top := ATop;
  Right := ARight;
...
procedure TdxRectFHelper.InitSize(const ALeft, ATop, AWidth, AHeight: Single);
begin
  Left := ALeft;
  Top := ATop;
  Right := ALeft + AWidth;
...
procedure TdxRectFHelper.Assign(const Value: TRect);
begin
  Left := System.Round(Value.Left);
  Top := System.Round(Value.Top);
  Right := System.Round(Value.Right);
  Bottom := System.Round(Value.Bottom);
end;

function TdxRectFHelper.CenterPoint: TdxPointF;
begin
  Result.X := (Left + Top) / 2;
  Result.Y := (Top + Bottom) / 2;
end;

procedure TdxRectFHelper.Empty;
begin
  Left := 0;
  Top := 0;
  Right := 0;
...
function TdxRectFHelper.IsEmpty: Boolean;
begin
  Result := (Left >= Right) or (Top >= Bottom);
end;

function TdxRectFHelper.IsZero : Boolean;
begin
  Result := Math.IsZero(Right) and Math.IsZero(Bottom) and Math.IsZero(Top) and Math.IsZero(Left);
end;

procedure TdxRectFHelper.Intersect(const R: TdxRectF);
begin
  if R.Left > Left then Left := R.Left;
  if R.Top > Top then Top := R.Top;
  if R.Right < Right then Right := R.Right;
...
class function TdxRectFHelper.Null: TdxRectF;
begin
  Result.Left := 0;
  Result.Top := 0;
  Result.Right := 0;
...
procedure TdxRectFHelper.Offset(X, Y: Single);
begin
  Left := Left + X;
  Right := Right + X;
  Top := Top + Y;
...
class function TdxSizeFHelper.Null: TdxSizeF;
begin
  Result.cx := 0;
  Result.cy := 0;
end;
