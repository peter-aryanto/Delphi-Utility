...
var
  AOutlen, ANewLines: Int64;
begin
  AOutlen := Int64(AInputLength) div 3 * 4;
  ...
  begin
    ANewLines := AOutlen div Base64LineBreakPosition;
    if (AOutlen mod Base64LineBreakPosition) = 0 then
      Dec(ANewLines);
    Inc(AOutlen, ANewLines * 2);
  end;
  if AOutlen > MaxInt then
    raise EOutOfMemory.Create('');
  Result := Integer(AOutlen);
end;
