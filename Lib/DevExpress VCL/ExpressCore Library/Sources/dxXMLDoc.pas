...
type
  ...
  { TdxXMLParserSharedStrings }

  TdxXMLParserSharedStrings = class
  strict private type
  ...
  strict private
    ...
    FSharedStrings: TdxXMLParserSharedStrings;
...
constructor TdxXMLParserSharedStrings.Create;
begin
  inherited Create;
  FTableSize := MaxWord;
...
destructor TdxXMLParserSharedStrings.Destroy;
begin
  Clear;
  SetLength(FTable, 0);
  inherited Destroy;
...
function TdxXMLParserSharedStrings.Add(const AToken: TdxXMLToken): AnsiString;
begin
  Result := DoAdd(AToken.Buffer, AToken.BufferLengthInChars);
end;

function TdxXMLParserSharedStrings.Add(const AValue: AnsiString): AnsiString;
begin
  Result := DoAdd(PAnsiChar(AValue), Length(AValue), @AValue);
end;

procedure TdxXMLParserSharedStrings.Clear;
var
  AItem: TValue;
  ATempItem: TValue;
  I: Integer;
...
function TdxXMLParserSharedStrings.DoAdd(P: PAnsiChar; L: Integer; AValue: PAnsiString = nil): AnsiString;
var
  AIndex: Integer;
  AItem: TValue;
  AHash: Cardinal;
...
{ TdxXMLParserSharedStrings.TValue }

constructor TdxXMLParserSharedStrings.TValue.Create(P: PAnsiChar; L: Integer; AHash: Cardinal; AValue: PAnsiString);
begin
  Hash := AHash;
  if AValue <> nil then
    Value := AValue^
...
begin
  inherited Create;
  FDocument := ADocument;
  FSharedStrings := TdxXMLParserSharedStrings.Create;
end;
      ...
      ttTagHeaderEnd:
        ...
        begin
          ...
          if (ATokenIndex = 0) then
            ANode.FName := FSharedStrings.Add(AToken);
          if (ATokenIndex = 2) and (AToken.TokenType <> ttEqual) then
            ATokenIndex := 1;
          if (ATokenIndex = 1) then
            ANode.Attributes.Add(FSharedStrings.Add(AToken), '');
          if (ATokenIndex = 3) then
            ANode.Attributes.Last.Value := FSharedStrings.Add(DecodeValue(TokenToString(AToken)));
        end;
    end;
    Inc(ATokenIndex);
  end;
...
begin
  Result := '';
  if (Length(Version) > 0) or (Length(Encoding) > 0) or (Length(StandAlone) > 0) then
  begin
    Result := '<?xml';
