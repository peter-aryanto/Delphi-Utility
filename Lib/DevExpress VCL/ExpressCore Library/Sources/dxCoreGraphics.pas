...
uses
  Windows, Messages, SysUtils, Classes, Graphics,  Generics.Defaults, Generics.Collections, dxCore, dxCoreClasses;
...
type
  ...
  {$ENDREGION}
    class var
      ...
      FNameToColorMapComparer: TCustomComparer<string>;
...
begin
  ...
  begin
    ...
    FNameToColorMap.AddOrSetValue(SystemColorsData[I].Name, AColor);
  end;
...
var
  AInfo: TColorInfo;
begin
  FNameToColorMap.Add(AName, AColor);
  AInfo.Color := AColor;
  AInfo.Name := AName;
...
begin
  FNameToColorMapComparer := TdxOrdinalIStringComparer.Create;
  FNameToColorMap := TDictionary<string, TdxAlphaColor>.Create(140 + 28, FNameToColorMapComparer);
...
begin
  if not FNameToColorMap.TryGetValue(AName, Result) then
    Result := Empty;
end;
...
begin
  ...
  if IsEmpty(Result) and not FHtmlNameToColorMap.TryGetValue(LowerCase(AHtmlColor), Result) then
    FNameToColorMap.TryGetValue(AHtmlColor, Result);
end;
...
constructor TdxSolidBrushCache.Create(ACapacity: Integer);
begin
  inherited Create;
  Capacity := ACapacity;
end;

destructor TdxSolidBrushCache.Destroy;
begin
  Capacity := 0;
  inherited Destroy;
end;

function TdxSolidBrushCache.GetBrush(AColor: TColor): HBRUSH;
var
  ACount: Integer;
  AItem: TItem;
  I: Integer;
begin
  ACount := Length(FItems);
  if ACount = 0 then
    raise EInvalidOperation.Create(ClassName);

  AColor := ColorToRGB(AColor);
  if InRange(FLastIndex, 0, ACount - 1) then
  begin
    AItem := FItems[FLastIndex];
    if (AItem <> nil) and (AItem.Color = AColor) then
      Exit(AItem.Handle);
  end;
  ...
  AItem := TItem.Create(AColor);
  FLastIndex := ACount - 1;
  FItems[FLastIndex] := AItem;
  Result := AItem.Handle;
end;

function TdxSolidBrushCache.GetCapacity: Integer;
begin
  Result := Length(FItems);
end;

procedure TdxSolidBrushCache.SetCapacity(AValue: Integer);
var
  I: Integer;
begin
  AValue := Max(AValue, 0);
...
{ TdxSolidBrushCache.TItem }

constructor TdxSolidBrushCache.TItem.Create(AColor: TColor);
begin
  Color := AColor;
  Handle := CreateSolidBrush(Color);
end;

destructor TdxSolidBrushCache.TItem.Destroy;
begin
  DeleteObject(Handle);
  inherited;
end;
