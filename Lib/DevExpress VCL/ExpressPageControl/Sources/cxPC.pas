...
procedure TcxTabViewInfo.CalculateImageAndTextHorizontalPositions;
  ...
  begin
    ...
    AText[ALength + 1] := #0;
    DrawText(Canvas.Handle, PChar(AText), -1, ARect, Painter.GetDTFlags(VisibleIndex) or DT_CALCRECT or DT_MODIFYSTRING);
    SetLength(AText, StrLen(PChar(AText)));
    Result := ARect.Right;
    if AText = '&...' then
      Result := 0
    else
      if Result > AAccessibleWidth then
        if Length(AText) > Length('....') then
        begin
          AText := Copy(AText, 1, Length(AText) - Length('...'));
          repeat
            Delete(AText, Length(AText), 1);
            Result := cxTextSize(Canvas.Font, AText + '...').cx;
            if Result > AAccessibleWidth then
              Result := 0;
          until (Length(AText) <= Length('....')) or (Result > 0);
        end
        else
          Result := 0;
  end;
...
begin
  PrepareCanvasFont(cxScreenCanvas);
  Result := cxTextSize(cxScreenCanvas.Font, Caption, Painter.GetCTFlags(VisibleIndex)).cx;
  cxScreenCanvas.Dormant;
end;
...
begin
  Result := (Properties.TabPosition in [tpLeft, tpRight]) and not ActuallyRotate or
    (Properties.TabPosition in [tpTop, tpBottom]) and ActuallyRotate;
end;
...
begin
  Result := DT_SINGLELINE or DT_END_ELLIPSIS;
...
begin
  Result := DT_SINGLELINE or DT_CALCRECT;
  if not TabViewInfo[ATabVisibleIndex].ShowAccelChar then
    Result := Result or DT_NOPREFIX;
end;
