...
type
  ...
  protected type
    TVariantComparer = class(TEqualityComparer<Variant>)
    public
...
{ TdxVariantToObjectDictionary.TVariantComparer }

function TdxVariantToObjectDictionary<TValue>.TVariantComparer.Equals(const Left, Right: Variant): Boolean;
begin
  Result := Left = Right; 
end;

function TdxVariantToObjectDictionary<TValue>.TVariantComparer.GetHashCode(const Value: Variant): Integer;
begin
...
begin
  if AOwnsValues then
    ...
    AOwnerships := [];
  inherited Create(AOwnerships, ACapacity, TVariantComparer.Create);
end;
